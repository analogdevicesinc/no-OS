...
adi_hal_Err_e no_os_HwOpen(void *devHalCfg)
{
	...
	gip_gpio_reset_n.number = ADRV9030_RESET_B;
...
FILE* __fopen(const char * filename, const char *mode)
{
	...
	if (!strcmp(filename, "ADRV9030_DeviceProfileTest.bin")) {
	...
	} else if (!strcmp(filename, "ADRV9030_stream_image.bin")) {
		...
		profile.start = profile.ptr = profile.data;
		profile.end = profile.start + length;
	} else if (!strcmp(filename, "ADRV9030_FW.bin")) {
		length = sizeof(ADRV9030_FW_bin);
		temp = (char *)no_os_calloc(length, sizeof(char));
		memcpy(temp, ADRV9030_FW_bin, sizeof(ADRV9030_FW_bin));
		profile.data = temp;
		profile.start = profile.ptr = profile.data;
		profile.end = profile.start + length;
	} else if (!strcmp(filename, "ADRV9030_RxGainTable.csv")) {
		length = strlen(ADRV9030_RxGainTable_text);
		temp = (char *)no_os_calloc(length, sizeof(char));
		strcpy(temp, ADRV9030_RxGainTable_text);
		profile.data = temp;
		profile.start = profile.ptr = profile.data;
