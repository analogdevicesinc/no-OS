...
int example_main()
{
	...
	/* write offset regs for calibration*/
	ret = adxl37x_write_reg(dev, ADXL37X_OFFSET_X, -3 & 0x0F);
	if (ret)
		goto error_dev;
	...
	/* read offset regs */
	ret = adxl37x_read_reg(dev, ADXL37X_OFFSET_X, &off_x);
	if (ret)
		goto error_dev;
	ret = adxl37x_read_reg(dev, ADXL37X_OFFSET_Y, &off_y);
	if (ret)
		goto error_dev;
	ret = adxl37x_read_reg(dev, ADXL37X_OFFSET_Z, &off_z);
	if (ret)
		goto error_dev;



	pr_info("offset trims expressed in two's complement: OFFSET_X=%d , OFFSET_Y=%d , OFFSET_Z=%d \n\r",
		off_x, off_y, off_z);
