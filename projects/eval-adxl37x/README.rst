...
demonstrates direct register reads and buffered FIFO acquisition over IIO
using the no-OS ADXL37x driver on the MAX32655FTHR platform.
...
VS and VREF are tied to the same rail: 3.3 V for the ADXL371 and 2.5 V for
the ADXL372. The MAX32655FTHR 3V3 rail supplies the ADXL371 directly; the
ADXL372 needs an external 2.5 V supply.
...
* Initializes the ADXL37x over SPI and disables the high-pass filter
* Writes the OFFSET_X/Y/Z trim registers and reads them back
* Continuously reads the X/Y/Z data registers with
  ``adxl37x_get_xyz_data`` and prints the raw and m/s² values over UART
...
.. warning::
	The selection of axes used through IIO is done through the main window of IIO oscilloscope.
...
+------------------+---------------------------+-------------------------+
| VS               | Power Supply              | 3V3 (ADXL371) /         |
|                  |                           | external 2.5 V (ADXL372)|
+------------------+---------------------------+-------------------------+
...
.. warning::
	**After connecting to ADXL through IIO**
