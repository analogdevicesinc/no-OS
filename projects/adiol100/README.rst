...
* **Port layer** (``adiol100_ilink_pl.c/h``) — Bridges the i-link stack
  to the ADIOL100 driver. Implements the ``iolink_hw_ops_t`` callbacks:
  baudrate/cycletime get/set, mode switching (including L+ enable and
  channel IRQ configuration), cycle timer control, FIFO messaging, and
  EstablishCommunication. An ISR registered with the driver signals
  FreeRTOS event groups to wake the DL thread on ADIOL100 interrupts.

* **OSAL** (``osal_freertos.c``) — FreeRTOS implementation of the i-link
  OS abstraction layer (mutexes, events, threads, mailboxes, timers).

* **Application layer** (``iolink_app.c/h``) — Generic IO-Link application
  built on top of the i-link SMI (Standardized Master Interface). Handles
  port configuration, device identification, process data callbacks,
  COMLOST recovery, and ISDU read/write helpers.
