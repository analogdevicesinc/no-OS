...
with an MCU board via the Pmod connector (J3):
...
calls the selected example's main function (e.g. ``basic_example_main()``). Shared
...
CubeMX ``.ioc`` files are provided: ``sdp-ck1z-basic.ioc`` (for the
basic example) and ``sdp-ck1z-ilink.ioc`` (for the ilink example;
disables SysTick/SVC/PendSV handler generation since FreeRTOS and the
application provide them).
