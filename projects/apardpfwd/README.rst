...
1. forward_packets_example (selected by default) - may be selected by setting the FORWARD_PACKETS_EXAMPLE = y (and all the other examples to n) in the main Makefile.
This example configures the output port of the AD-APARDPFWD-SL and pings the downstream device. It initializes the ADIN1110 Ethernet controller and reads its device ID.

2. apard_communication_example - may be selected by setting the APARD_COMMUNICATION_EXAMPLE = y (and all the other examples to n) in the main Makefile.
This example configures the output port of the AD-APARDPFWD-SL and opens a TCP socket to communicate with the AD-APARD32690-SL. It will start a TCP server using the interface ADIN1110 is connected to (the default settings are IP: 192.168.97.50 port: 10000). It will reply back to the connected client with the
