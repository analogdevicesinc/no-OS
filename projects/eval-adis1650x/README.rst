...
`Project Common Data Path <https://github.com/analogdevicesinc/no-OS/tree/main/projects/eval-adis1650x/src/common>`_

The macros used in Common Data are defined in platform specific files found in:
`Project Platform Configuration Path <https://github.com/analogdevicesinc/no-OS/tree/main/projects/eval-adis1650x/src/platform>`_

Basic example
^^^^^^^^^^^^^

This is a simple example which initializes the adis1650x selected device and
...
This project is actually a IIOD demo for EVAL-ADIS1650X device series.
...
`IIO Trigger Example <https://github.com/analogdevicesinc/no-OS/tree/main/projects/eval-adis1650x/src/examples/iio_trigger_example>`_
...
ADIS1650X offers the capability to use DATA_READY pin as a flag which shows when
...
+---------------------------+----------+-------------------------------------------------------+-------------------+
| EVAL-ADIS1650X Pin Number | Mnemonic | Function                                              | SDP-K1 Pin Number |
+---------------------------+----------+-------------------------------------------------------+-------------------+
...
+---------------------------+----------+-------------------------------------------------------+---------------------+
| EVAL-ADIS1650X Pin Number | Mnemonic | Function                                              | MAX78000 Pin Number |
+---------------------------+----------+-------------------------------------------------------+---------------------+
...
* ``basic`` - basic example (``eval-adis1650x/basic.conf``)
* ``iio_trigger`` - IIO trigger example (``eval-adis1650x/iio_trigger.conf``)

Below is a consolidated set of commands to configure, build, flash, and debug
the eval-adis1650x project on the ``rpi-pico`` board (swap the ``.conf`` file to build a
different variant):

.. code-block:: bash

        # Configure: select the rpi-pico preset and the basic example variant.
        # Pass -DPROBE=openocd (default) or -DPROBE=jlink to enable the flash
        # and debug targets.
        cmake --preset rpi-pico -B build-eval-adis1650x-basic-rpi-pico \
            -DPROJECT_DEFCONFIG=eval-adis1650x/basic.conf -DPROBE=openocd

        # Build: compile the project (produces .elf and, when picotool is
        # available, a .uf2 image under the build directory).
        cmake --build build-eval-adis1650x-basic-rpi-pico --target eval-adis1650x

        # Change Kconfig options interactively (optional).
        cmake --build build-eval-adis1650x-basic-rpi-pico --target menuconfig

        # Flash: program the firmware onto the board via the selected probe.
        cmake --build build-eval-adis1650x-basic-rpi-pico --target flash

        # Debug: start OpenOCD and attach GDB.
        cmake --build build-eval-adis1650x-basic-rpi-pico --target debug