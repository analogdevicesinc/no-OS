/*
 * Project-specific lwipopts.h for the adin1140 iperf_lwip_freertos example.
...
/* NO_SYS==1: Use lwIP without OS-awareness (raw API only).
 * SYS_LIGHTWEIGHT_PROT=1: protect critical sections via taskENTER_CRITICAL()
 * when running under FreeRTOS. */
#define NO_SYS                          1
...
#define SYS_ARCH_PROTECT(lev)           do { (void)(lev); taskENTER_CRITICAL(); } while (0)
