...
exercise the no-OS ADIN1140 driver in two modes:
...
``nucleo-h563zi``). See the Build Command sections under each supported
platform below for the full invocation.
...
host MCU. The ISR sets a global ``irq_triggered`` flag; the main loop
calls ``no_os_lwip_step()`` and clears the flag only when the MAC-PHY has
signalled work, so SPI traffic is driven by the hardware rather than by
polling.
...
ISR masks the pin at the GPIO controller and uses
``xTaskNotifyFromISR()`` to wake the network task. The task blocks on
``ulTaskNotifyTake()``, calls ``no_os_lwip_step()``, then re-enables the
GPIO IRQ. This is the
...
* MAX32690FTHR (``TARGET=max32690``) Maxim host board

Connections
^^^^^^^^^^^

Common control signals (UART log + ADIN1140 SPI + INT) are wired as
follows. SPI uses no-OS chip-select index 1, mode 0.
