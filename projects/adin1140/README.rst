...
The example to build is selected via the ``EXAMPLE`` variable (either in
the project's
`Makefile <https://github.com/analogdevicesinc/no-OS/blob/main/projects/adin1140/Makefile>`__
or on the ``make`` command line). The default is ``iperf_lwip_example``.
...
In order to build the ``frame_rx_tx`` example, make sure you have the
...
In order to build the ``iperf_lwip_example``, make sure you have the
following configuration in the
`Makefile <https://github.com/analogdevicesinc/no-OS/blob/main/projects/adin1140/Makefile>`__:
...
In order to build the ``iperf_lwip_freertos`` example, make sure you
have the following configuration in the
`Makefile <https://github.com/analogdevicesinc/no-OS/blob/main/projects/adin1140/Makefile>`__:

.. code-block:: bash

   # Select the example
   EXAMPLE = iperf_lwip_freertos
...
project CFLAGS (default ``192.168.97.40``). Connect with, for example::

   iio_info -u ip:192.168.97.40

In order to build the ``iio_lwip_example``, make sure you have the
following configuration in the
`Makefile <https://github.com/analogdevicesinc/no-OS/blob/main/projects/adin1140/Makefile>`__:
...
.. code-block:: bash
   ...
   make reset
   # to build the project (defaults to EXAMPLE=iperf_lwip_example)
   make PLATFORM=maxim TARGET=max32690
   # or, for the MAX78000FTHR
   make PLATFORM=maxim TARGET=max78000
   # to select a different example
   make PLATFORM=maxim TARGET=max32690 EXAMPLE=iio_lwip_example
   # to flash the code
   make run
