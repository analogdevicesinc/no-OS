...
f_rename() /      are removed. /   2: f_opendir(), f_readdir() and f_closedir()
are removed in addition to 1. /   3: f_lseek() function is removed in addition
to 2. */
...
features want C99 or later. /  When FF_LFN_UNICODE >= 1 with LFN enabled,
string functions convert the character /  encoding in it. FF_STRF_ENCODE
selects assumption of character encoding ON THE FILE /  to be read/written via
those functions.
/
...
to fully support LFN /  specification. /  When use stack for the working
...
suffcient for /  the file names to read. The maximum possible length of the
...
mounted. /  When this function is enabled (1), each logical drive number can be
bound to /  arbitrary physical drive and partition listed in the VolToPart[].
...
this /  option, and f_getfree() function at the first time after volume mount
...
objects. /  >0: Enable file lock function. The value defines how many
...
ff_mutex_give() /      function, must be added to the project. Samples are
available in ffsystem.c.
/
