 ...
 * \param indent the indentation level for the log message
 ...
 * These are stub functions that return success to allow the code to compile
 * and run in single-threaded environments.
 */
adi_hal_Err_e no_os_mutex_init_wrapper(adi_hal_mutex_t* const mutex)
{
	/* Mutex not implemented for this platform - single threaded operation */
	(void)mutex;
...
adi_hal_Err_e no_os_HwReset(void *devHalCfg, const uint8_t pinLevel)
{
	...
	phal = (struct adrv904x_hal_cfg *)devHalCfg;
