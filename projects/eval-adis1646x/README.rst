...
* ``basic`` - basic example (``eval-adis1646x/basic.conf``)
* ``iio_trigger`` - IIO trigger example (``eval-adis1646x/iio_trigger.conf``)

Below is a consolidated set of commands to configure, build, flash, and debug
the eval-adis1646x project on the ``rpi-pico`` board (swap the ``.conf`` file to build a
different variant):
...
.. code-block:: bash
        ...
        # and debug targets.
        cmake --preset rpi-pico -B build-eval-adis1646x-basic-rpi-pico \
            -DPROJECT_DEFCONFIG=eval-adis1646x/basic.conf -DPROBE=openocd

        # Build: compile the project (produces .elf and, when picotool is
        # available, a .uf2 image under the build directory).
        cmake --build build-eval-adis1646x-basic-rpi-pico --target eval-adis1646x

        # Change Kconfig options interactively (optional).
        cmake --build build-eval-adis1646x-basic-rpi-pico --target menuconfig

        # Flash: program the firmware onto the board via the selected probe.
        cmake --build build-eval-adis1646x-basic-rpi-pico --target flash

        # Debug: start OpenOCD and attach GDB.
        cmake --build build-eval-adis1646x-basic-rpi-pico --target debug