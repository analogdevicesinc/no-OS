Cora Z7 Ethernet / lwIP Demo
=============================

Overview
--------

This project demonstrates bare-metal Ethernet networking on the Digilent
Cora Z7 board using the Zynq-7000 PS GEM (XEmacPs) and the lwIP raw API
(NO_SYS=1). Two example applications are available:
...
- Xilinx Vitis / Vivado 2022.2 (``xsct`` and ``vitis`` must be in PATH)
- A hardware description file (``*.xsa``) generated from a Vivado project
  targeting the Cora Z7-07S or Cora Z7-10
...
Place the ``.xsa`` file inside ``projects/cora_z7/`` before building.
...
Build outputs are written to ``projects/cora_z7/build/``.
...
.. code-block:: bash
   ...
   make PLATFORM=xilinx NO_OS_IP=169.254.3.1 NO_OS_NETMASK=255.255.0.0 NO_OS_GATEWAY=0.0.0.0
