...
/**
 * @brief Categorized skip that releases the test's hardware on the way out.
 *
 * A skip decided after the test has already brought hardware up must undo it,
 * exactly like a failing assertion does. Expands the enclosing CLEANUP; see the
 * _OR_CLEANUP assertion forms below.
 */
...
/*
 * -----------------------------------------------------------------------------
 * Assertions, and the _OR_CLEANUP forms.
 *
 * A failing assertion returns from the test immediately. That is fine while the
 * test owns nothing, but once it has started a timer, unmasked an interrupt or
 * opened a port, returning early leaves that hardware live for every later test
 * in the run -- and the failure that follows points at the wrong test.
 *
 * Each assertion therefore exists in two forms built from one shared _ACT body:
 *
 *   TEST_ASSERT_EQ(a, b, "NAME")              plain; nothing to undo
 *   TEST_ASSERT_EQ_OR_CLEANUP(a, b, "NAME")   expands CLEANUP, then returns -1
 *
 * CLEANUP is not a parameter: the _OR_CLEANUP forms name it directly, so it must
 * be #defined by the test file around every function that uses one. It expands
 * inside the function body and can therefore refer to that function's locals.
 * Keep it idempotent and NULL-guarded so a single definition stays valid at
 * every point in the test, and reuse one definition across neighbouring tests
 * that own the same hardware:
 *
 *   #define CLEANUP \
 *           if (timer != NULL) { \
 ...
 *   #undef CLEANUP
 *
 * Return codes inside CLEANUP are deliberately discarded: the failure being
 * reported is the one that got us here, and a teardown error must not overwrite
 * it. The same CLEANUP is also called on the success path, so the test releases
 * what it acquired however it exits.
 * -----------------------------------------------------------------------------
 */

/** @brief Shared body: assert a condition, run @p on_fail before returning. */
...
/** @brief Assert a condition; release the test's hardware on failure. */
#define TEST_ASSERT_OR_CLEANUP(condition, name) \
	TEST_ASSERT_ACT(condition, name, CLEANUP)

/** @brief Shared body: assert equality, run @p on_fail before returning. */
...
/** @brief Assert equality; release the test's hardware on failure. */
#define TEST_ASSERT_EQ_OR_CLEANUP(actual, expected, name) \
	TEST_ASSERT_EQ_ACT(actual, expected, name, CLEANUP)

/** @brief Shared body: assert inequality, run @p on_fail before returning. */
...
/** @brief Assert inequality; release the test's hardware on failure. */
#define TEST_ASSERT_NEQ_OR_CLEANUP(actual, expected, name) \
	TEST_ASSERT_NEQ_ACT(actual, expected, name, CLEANUP)

/** @brief Shared body: assert an inclusive range, run @p on_fail on failure. */
...
/** @brief Assert an inclusive range; release the hardware on failure. */
...
#define TEST_ASSERT_COMPARE_ACT(actual, expected, name, invalid_op, relation, \
				on_fail) \
	do { \
		long long _test_actual = (long long)(actual); \
		long long _test_expected = (long long)(expected); \
...
/** @brief Relational assertion that returns on failure. */
...
/** @brief Assert actual > expected; release the hardware on failure. */
...
/** @brief Shared body: accept 0 or -ENOTSUP, run @p on_fail otherwise. */
...
/** @brief Accept success or unsupported; release the hardware otherwise. */
...
/** @brief Shared body: assert a tolerance band, run @p on_fail on failure. */
...
/** @brief Assert a tolerance band; release the hardware on failure. */
...
/** @brief Shared body: assert counter direction, run @p on_fail on failure. */
...
/** @brief Assert counter direction; release the hardware on failure. */
