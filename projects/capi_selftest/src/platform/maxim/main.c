 ...
 * @return 0 on success, -1 if board has no GPIO-IRQ path.
 */
 ...
 * @return true if the input pin's flag was set and was cleared, false otherwise.
 *
 * Note: Checks the loopback input pin's latched interrupt flag and clears it
 * only if set, returning whether this pin was the source.
 */
bool platform_gpio_irq_ack(void)
{
	 ...
	 */
	uint32_t mask = 1U << gpio_input_pin_numbers[0];

	if (MXC_GPIO_GetFlags(MXC_GPIO0) & mask) {
		MXC_GPIO_ClearFlags(MXC_GPIO0, mask);
		return true;
	}
