...
int example_main(void)
{
	...
	float temperature;
	double impedance;
	double gain_factors[BASIC_INC_NUM + 1];
	int16_t real, imag;
	int imp_milli;
	int freq;
	int c;
	...
	if (measurement_mode == AD5933_MEAS_MODE_2W) {
		...
		printf("Connect the calibration impedance to P14.\r\n");
		...
		printf("Connect the calibration impedance to J5.\r\n");
	...
	for (int i = 0; i <= BASIC_INC_NUM; i++) {
		...
		ret = ad5933_calculate_gain_factor(dev, &gain_factors[i],
						   calibration_impedance, measurement_mode);
		if (ret)
			goto error_remove;

		printf("Frequency: %d Hz, Gain Factor: %.12f\r\n\r\n", freq, gain_factors[i]);
	}

	printf("Connect the unknown impedance.\r\n");
	...
	while (true) {
		...
		for (int i = 0; i <= BASIC_INC_NUM; i++) {
			...
			ret = ad5933_calculate_impedance(dev, gain_factors[i], &impedance,
							 measurement_mode);
			if (ret)
				goto error_remove;
			...
			printf("Frequency: %d Hz, Impedance: %d.%03d\r\n\r\n", freq, imp_milli / 1000,
			       imp_milli % 1000);
		}
	}
