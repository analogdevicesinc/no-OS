...
static int test_register_operations(struct max20362_dev *dev)
{
	...
	if (ret) {
		print_test_result("Register Read", false, NULL);
		return ret;
	}
	...
	ret = max20362_reg_read(dev, test_reg, &read_val);
	if (ret) {
		print_test_result("Read Bits", false, NULL);
		return ret;
	}
	...
	ret = max20362_reg_write(dev, test_reg, orig_val);
	if (ret) {
		print_test_result("Write Original Value", false, NULL);
		return ret;
	}
...
static int test_buck_boost_functions(struct max20362_dev *dev)
{
	...
	if (ret) {
		print_test_result("Read BB Config", false, NULL);
		return ret;
	}
	...
	if (ret) {
		print_test_result("Get Status", false, NULL);
		return ret;
	}
	...
	/* Disable for safety */
	ret = max20362_enable_buck_boost(dev, false);
	if (ret) {
		print_test_result("Disable Buck-Boost", false, NULL);
		return ret;
	}
	no_os_mdelay(10);
...
static int test_ldo_functions(struct max20362_dev *dev)
{
	...
	if (ret) {
		print_test_result("Read LDO VSet", false, NULL);
		return ret;
	}
	...
	if (ret) {
		print_test_result("Read LDO Config", false, NULL);
		return ret;
	}
	...
	ret = max20362_reg_read(dev, MAX20362_REG_LDO_CFG, &reg_val);
	if (ret) {
		print_test_result("Re-read LDO Config", false, NULL);
		return ret;
	}
	sprintf(detail_buf, "Enable bit: %s",
	...
	if (ret) {
		print_test_result("Disable LDO", false, NULL);
		return ret;
	}
	no_os_mdelay(10);
...
static int test_dvs_modes(struct max20362_dev *dev)
{
	...
	if (ret) {
		print_test_result("Read DVS Config", false, NULL);
		return ret;
	}
	...
	/*
	 * Round-Robin setup per datasheet sequence:
	 * 1. Configure RRWrap, RRClkPolarity, RRTimeoutRes before enabling RR
	 * 2. Program RR voltage table (this also enables RR mode)
	 */

	/* Step 1a: Configure RR wrap (wrap back to first entry) */
	pr_info("Testing RR wrap configuration...\n");
	ret = max20362_set_rr_wrap(dev, true);
	if (ret) {
		print_test_result("Set RR Wrap", false, NULL);
		return ret;
	}

	ret = max20362_reg_read(dev, MAX20362_REG_DVS_CFG, &reg_val);
	if (ret) {
		print_test_result("Read DVS Config (wrap)", false, NULL);
		return ret;
	}
	sprintf(detail_buf, "RRWrap: %s",
		(reg_val & MAX20362_DVSCFG_RRWRAP_MSK) ? "WRAP" : "STAY");
	print_test_result("RR Wrap Verification",
			  (reg_val & MAX20362_DVSCFG_RRWRAP_MSK) != 0, detail_buf);

	/* Step 1b: Configure RR clock polarity (rising edge) */
	pr_info("Testing RR clock polarity...\n");
	ret = max20362_set_rr_clk_polarity(dev, true);
	if (ret) {
		print_test_result("Set RR Clk Polarity", false, NULL);
		return ret;
	}

	ret = max20362_reg_read(dev, MAX20362_REG_RR_CFG1, &reg_val);
	if (ret) {
		print_test_result("Read RRCfg1 (polarity)", false, NULL);
		return ret;
	}
	sprintf(detail_buf, "RRClkPolarity: %s",
		(reg_val & MAX20362_RRCFG1_RRCLKPOLARITY_MSK) ? "RISING" : "FALLING");
	print_test_result("RR Clk Polarity Verification",
			  (reg_val & MAX20362_RRCFG1_RRCLKPOLARITY_MSK) != 0,
			  detail_buf);

	/* Step 1c: Configure RR timeout reset (enable) */
	pr_info("Testing RR timeout reset...\n");
	ret = max20362_set_rr_timeout_reset(dev, true);
	if (ret) {
		print_test_result("Set RR Timeout Reset", false, NULL);
		return ret;
	}

	ret = max20362_reg_read(dev, MAX20362_REG_RR_CFG1, &reg_val);
	if (ret) {
		print_test_result("Read RRCfg1 (timeout)", false, NULL);
		return ret;
	}
	sprintf(detail_buf, "RRTimeoutRes: %s",
		(reg_val & MAX20362_RRCFG1_RRTIMEOUTRES_MSK) ? "RESET" : "NO RESET");
	print_test_result("RR Timeout Reset Verification",
			  (reg_val & MAX20362_RRCFG1_RRTIMEOUTRES_MSK) != 0,
			  detail_buf);

	/* Step 2: Program Round-Robin table (also enables RR mode) */
	...
	ret = max20362_reg_read(dev, MAX20362_RR_VSET_ADDR(0), &reg_val);
	if (ret) {
		print_test_result("Read RR0", false, NULL);
		return ret;
	}
	...
	/* Verify Round-Robin mode was enabled by set_dvs_rr_table */
	pr_info("Verifying Round-Robin mode enabled...\n");
	ret = max20362_reg_read(dev, MAX20362_REG_DVS_CFG, &reg_val);
	if (ret) {
		print_test_result("Read DVS Config (RR)", false, NULL);
		return ret;
	}
	...
	ret = max20362_set_dvs_mode(dev, MAX20362_DVS_SOURCE_I2C);
	if (ret) {
		print_test_result("Restore DVS I2C Mode", false, NULL);
		return ret;
	}
...
static int test_interrupt_handling(struct max20362_dev *dev)
{
	...
	if (ret) {
		print_test_result("Get INT Status", false, NULL);
		return ret;
	}
	...
	if (ret) {
		print_test_result("Read LDO INT", false, NULL);
		return ret;
	}
	...
	if (ret) {
		print_test_result("Read INGEN INT", false, NULL);
		return ret;
	}
	...
	ret = max20362_set_int_mask(dev, 0xFF);
	if (ret) {
		print_test_result("Set INT Mask", false, NULL);
		return ret;
	}
	ret = max20362_set_ldo_int_mask(dev, 0xFF);
	if (ret) {
		print_test_result("Set LDO INT Mask", false, NULL);
		return ret;
	}
	ret = max20362_set_ingen_int_mask(dev, 0xFF);
	if (ret) {
		print_test_result("Set INGEN INT Mask", false, NULL);
		return ret;
	}
...
static int test_protection_features(struct max20362_dev *dev)
{
	...
	if (ret) {
		print_test_result("Lock Register Read", false, NULL);
		return ret;
	}
	...
	ret = max20362_reg_read(dev, MAX20362_REG_LOCK_MSK, &reg_val);
	if (ret) {
		print_test_result("Lock Register Re-read", false, NULL);
		return ret;
	}
	sprintf(detail_buf, "Lock mask: 0x%02X (BBLck bit %s)",
		reg_val, (reg_val & 0x01) ? "SET" : "CLEAR");
...
static int test_capacitor_voltage(struct max20362_dev *dev)
{
	...
	if (ret) {
		print_test_result("Read Cap Voltage", false, NULL);
		return ret;
	}
	...
	ret = max20362_reg_read(dev, MAX20362_REG_CAP_VSET, &reg_val);
	if (ret) {
		print_test_result("Re-read Cap Voltage", false, NULL);
		return ret;
	}
...
static int test_current_limiting(struct max20362_dev *dev)
{
	...
	if (ret) {
		print_test_result("Read Ingenuity Config", false, NULL);
		return ret;
	}
	...
	ret = max20362_reg_read(dev, MAX20362_REG_IGN_CFG, &reg_val);
	if (ret) {
		print_test_result("Re-read Ingenuity Config", false, NULL);
		return ret;
	}
