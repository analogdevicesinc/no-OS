...
static void test_set_led_pulse_amplitude_invalid_led(struct maxm86161_dev *dev)
{
	...
	TEST_ASSERT_EQUAL(-EINVAL,
			  maxm86161_set_led_pulse_amplitude(dev, 5, 0x20));
}
...
static void test_led_pilot_pa_roundtrip(struct maxm86161_dev *dev)
{
	...
	TEST_ASSERT_EQUAL(0,
			  maxm86161_set_led_pulse_amplitude(dev,
					  MAXM86161_LED_NUM_PILOT_GREEN, 0x3F));
	TEST_ASSERT_EQUAL(0,
			  maxm86161_get_led_pulse_amplitude(dev,
					  MAXM86161_LED_NUM_PILOT_GREEN, &val));
...
static void test_picket_fence_roundtrip(struct maxm86161_dev *dev)
{
	...
	TEST_ASSERT_EQUAL(0, maxm86161_set_pf_enable(dev, true));
	TEST_ASSERT_EQUAL(0, maxm86161_set_pf_order(dev, true));
	TEST_ASSERT_EQUAL(0, maxm86161_set_pf_iir_tc(dev, 1));
	TEST_ASSERT_EQUAL(0, maxm86161_set_pf_iir_init(dev, 2));
	TEST_ASSERT_EQUAL(0, maxm86161_set_pf_threshold_sigma(dev, 1));

	TEST_ASSERT_EQUAL(0, maxm86161_get_pf_enable(dev, &enable));
	TEST_ASSERT_EQUAL(0, maxm86161_get_pf_order(dev, &order));
	TEST_ASSERT_EQUAL(0, maxm86161_get_pf_iir_tc(dev, &iir_tc));
	TEST_ASSERT_EQUAL(0, maxm86161_get_pf_iir_init(dev, &iir_init_value));
	TEST_ASSERT_EQUAL(0, maxm86161_get_pf_threshold_sigma(dev,
			  &threshold_sigma_mult));

	TEST_ASSERT_TRUE(enable);
	TEST_ASSERT_TRUE(order);
	TEST_ASSERT_EQUAL(1, iir_tc);
	TEST_ASSERT_EQUAL(2, iir_init_value);
	TEST_ASSERT_EQUAL(1, threshold_sigma_mult);
}
...
static void test_die_temperature_read(struct maxm86161_dev *dev)
{
	...
	TEST_ASSERT_EQUAL(0, maxm86161_set_shutdown(dev, true));
}
...
int example_main(void)
{
	...
	pr_info("MAXM86161 initialized. Part ID: 0x%02X Rev: 0x%02X\n",
		MAXM86161_PART_ID_VAL, dev->rev_id);
