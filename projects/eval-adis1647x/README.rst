...
`Project Common Data Path <https://github.com/analogdevicesinc/no-OS/tree/main/projects/eval-adis1647x/src/common>`_

The macros used in Common Data are defined in platform specific files found in:
`Project Platform Configuration Path <https://github.com/analogdevicesinc/no-OS/tree/main/projects/eval-adis1647x/src/platform>`_
...
`IIO Trigger Example <https://github.com/analogdevicesinc/no-OS/tree/main/projects/eval-adis1647x/src/examples/iio_trigger_example>`_
...
* ``basic`` - basic example (``eval-adis1647x/basic.conf``)
* ``iio_trigger`` - IIO trigger example (``eval-adis1647x/iio_trigger.conf``)

Below is a consolidated set of commands to configure, build, flash, and debug
the eval-adis1647x project on the ``rpi-pico`` board (swap the ``.conf`` file to build a
different variant):
...
.. code-block:: bash
        ...
        # and debug targets.
        cmake --preset rpi-pico -B build-eval-adis1647x-basic-rpi-pico \
            -DPROJECT_DEFCONFIG=eval-adis1647x/basic.conf -DPROBE=openocd

        # Build: compile the project (produces .elf and, when picotool is
        # available, a .uf2 image under the build directory).
        cmake --build build-eval-adis1647x-basic-rpi-pico --target eval-adis1647x

        # Change Kconfig options interactively (optional).
        cmake --build build-eval-adis1647x-basic-rpi-pico --target menuconfig

        # Flash: program the firmware onto the board via the selected probe.
        cmake --build build-eval-adis1647x-basic-rpi-pico --target flash

        # Debug: start OpenOCD and attach GDB.
        cmake --build build-eval-adis1647x-basic-rpi-pico --target debug
