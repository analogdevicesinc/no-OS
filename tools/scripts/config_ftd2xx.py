...
def parse_args():
	...
	parser.add_argument('--arch', choices=sorted(D2XX_LINUX_URLS.keys()),
		help="Override the auto-detected Linux architecture. Needed to "
		     "install libraries for an arch other than the host's, e.g. "
		     "installing x86_32 libs on an x86_64 host for a -m32 build.")
...
def main():
	...
	if plat_key == 'linux':
		if args.arch:
			mpsse_arch = d2xx_arch = args.arch
			print("Using arch override: %s" % d2xx_arch)
		else:
