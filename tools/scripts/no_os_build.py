...
def _run_cmd(cmd, cwd, timeout, capture=True, check=True):
    """Run cmd with a hard timeout, SIGKILLing the whole process group on expiry.

    Reaps a stuck vitis/java tree spawned by cmake instead of orphaning it.
    """
    ...
    with subprocess.Popen(cmd, **kwargs) as proc:
        try:
            ...
            # start_new_session makes proc.pid the group id; kill the whole
            # group so spawned vitis/java servers die too.
            try:
