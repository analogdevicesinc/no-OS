...
def main(device_name, firmware_file, start_addr):
    ...
    try:
        ...
        # jlink may still be None if pylink.JLink() itself failed (e.g. the
        # J-Link SDK/DLL could not be located), which raises something other
        # than JLinkException. Guard the close so it does not mask the real
