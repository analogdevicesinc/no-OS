...
def error(msg):
        print(f"generate_config.py: error: {msg}", file=sys.stderr)
        sys.exit(1)
...
kconfig_path = src_dir.joinpath("Kconfig")
if not kconfig_path.exists():
        error(f"Kconfig file not found: {kconfig_path}")

try:
        kconf = kconfiglib.Kconfig(str(kconfig_path))
except kconfiglib.KconfigError as e:
        error(f"failed to parse {kconfig_path}:\n{e}")
...
try:
        ...
        error(f"failed to load {main_config}:\n{e}")
        ...
        if args.defconfig is not None:
                ...
                for defconfig in args.defconfig:
                        ...
                        if not defconfig_path.exists():
                                error(f"defconfig file not found: {defconfig_path}")
                        try:
                                kconf.load_config(str(defconfig_path), replace=False)
                        except kconfiglib.KconfigError as e:
                                error(f"failed to load defconfig {defconfig_path}:\n{e}")
...
try:
        ...
        with open(cmake_tmp, "w") as cmake_file:
                ...
                for sym in kconf.unique_defined_syms:
                        # Skip symbols kconfiglib itself would not write out
                        # (dependency-unmet, i.e. config_string is empty), the
                        # same way the C header below does. Without this, a
                        # dependency-gated symbol such as NO_OS_IP (depends on
                        # NO_OS_STATIC_IP) would emit `set(CONFIG_NO_OS_IP )`
                        # even when static IP is disabled, leaking an inactive
                        # symbol (and any non-empty default) into the CMake
                        # config and diverging from no_os_config.h.
...
try:
        ...
        with open(header_tmp, "w") as hf:
                ...
                for sym in kconf.unique_defined_syms:
                        # Skip symbols kconfiglib itself would not write out,
                        # i.e. those whose dependencies are unmet (config_string
                        # is empty). Without this, a dependency-gated STRING such
                        # as NO_OS_IP (depends on NO_OS_STATIC_IP) would still
                        # emit `#define CONFIG_NO_OS_IP ""` even when static IP is
                        # disabled, making `#ifdef CONFIG_NO_OS_IP` guards always
                        # true and silently selecting the static-IP code path.
                        if not sym.config_string:
                                continue
                        if sym.orig_type is kconfiglib.STRING:
                                val = f'"{sym.str_value}"'
