...
HARDWARE ?= $(wildcard *.xsa) $(wildcard *.hdf) $(wildcard *.sopcinfo) $(wildcard *.ioc) $(wildcard pinmux_config.c) $(wildcard *sge.zip) $(wildcard *sge) $(wildcard *sge/)
#If platform not set get it from HARDWARE file
ifneq '' '$(findstring sge.zip,$(HARDWARE))'
PLATFORM = lattice
else
HARDWARE_NO_TRAILING_SLASH := $(patsubst %/,%,$(HARDWARE))
ifneq '' '$(filter %/sge sge,$(HARDWARE_NO_TRAILING_SLASH))'
PLATFORM = lattice
else
