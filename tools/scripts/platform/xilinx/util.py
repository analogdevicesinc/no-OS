...
def _create_hw0_platform(out_dir, xsa, vcpu, attempts=2):
    ...
    for attempt in range(1, attempts + 1):
        ...
        try:
            ...
            print(f"WARNING: Vitis error during platform creation: {exc}",
                  file=sys.stderr)
        ...
        try:
            ...
            pass
        if attempt < attempts:
            print(f"WARNING: platform 'hw0' not produced on attempt "
                  f"{attempt}/{attempts} (FSBL getsupported_comp assist is the "
                  f"usual culprit); retrying on a clean workspace...",
                  file=sys.stderr)
