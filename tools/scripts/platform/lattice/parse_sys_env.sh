...
ignore_arch_check=${5:-n}
ignore_version_check=${6:-n}
ignore_cpu_version_check=${7:-n}
...
if [ "$arch_from_ext" != "$arch_req" ]; then
    if [ "$(lc "$ignore_arch_check")" = "y" ]; then
        echo "arch mismatch ignored: sys_env encodes $arch_from_ext" \
...
if [ "$propel_version" != "$req_propel" ]; then
    if [ "$(lc "$ignore_version_check")" = "y" ]; then
        echo "version mismatch ignored: Propel is $propel_version, expected $req_propel" >&2
    else
...
if [ "$riscv_version" != "$req_riscv_rtos" ]; then
    if [ "$(lc "$ignore_cpu_version_check")" = "y" ]; then
        echo "version mismatch ignored: riscv_rtos is $riscv_version, expected $req_riscv_rtos" >&2
    else
