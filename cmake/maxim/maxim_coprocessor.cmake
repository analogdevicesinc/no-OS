...
#         DEBUG_INFO                       # retain GDB line information (optional)
#         OPTIMIZATION O2                  # GCC optimization suffix (optional)
#         SRAM2_RESERVE 0x1000)            # reserve low CPU1 SRAM (optional)
#
...
# Locate the RISC-V cross toolchain shipped with the Maxim SDK. Sets RISCV_GCC,
# RISCV_OBJCOPY and RISCV_NM in the caller's scope. The ARM toolchain resolved
...
function(maxim_find_riscv_toolchain)
    ...
    set(RISCV_NM ${_bindir}/riscv-none-elf-nm PARENT_SCOPE)
endfunction()

function(maxim_add_riscv_coprocessor ARM_TARGET)
    cmake_parse_arguments(RV "DEBUG_INFO"
        "FORCE_INCLUDE;OPTIMIZATION;SRAM2_RESERVE"
    ...
    endforeach()
    if(RV_OPTIMIZATION)
        set(_optimization -${RV_OPTIMIZATION})
    else()
        set(_optimization -Os)
    endif()
    set(_debug_flags "")
    if(RV_DEBUG_INFO)
        set(_debug_flags -g3)
    endif()
    set(_cflags ${_march} ${_mabi} ${_optimization} ${_debug_flags}
                -ffunction-sections -fdata-sections
                ${_defs} ${_incs})

    # OpenOCD needs a small SRAM work area while debugging/flashing CPU1.  A
    # project can reserve bytes at the bottom of SRAM2; the generated common.ld
    # shadows the SDK's memory map because _bin precedes _gcc in the link path.
    if(RV_SRAM2_RESERVE)
        math(EXPR _rv_sram2_origin "0x20010000 + ${RV_SRAM2_RESERVE}"
             OUTPUT_FORMAT HEXADECIMAL)
        math(EXPR _rv_sram2_length "0x0000C000 - ${RV_SRAM2_RESERVE}"
             OUTPUT_FORMAT HEXADECIMAL)
        configure_file(
            ${CMAKE_CURRENT_FUNCTION_LIST_DIR}/templates/common_riscv_memory.ld.in
            ${_bin}/common.ld @ONLY)
    else()
        # Do not let a reserve setting from an earlier configure survive after
        # a project removes SRAM2_RESERVE while reusing the build directory.
        file(REMOVE ${_bin}/common.ld)
    endif()
    ...
    if(RV_NOOS_DRIVERS)
        ...
        list(APPEND _noos_cflags
             ...
             -I${_mxc_src}/CAMERAIF -I${_mxc_src}/PT)
    ...
    get_target_property(_arm_defs ${ARM_TARGET} COMPILE_DEFINITIONS)
    get_target_property(_arm_opts ${ARM_TARGET} COMPILE_OPTIONS)
    get_target_property(_arm_link_opts ${ARM_TARGET} LINK_OPTIONS)
    ...
    if(_arm_defs)
        target_compile_definitions(${_premap} PRIVATE ${_arm_defs})
    endif()
    if(_arm_opts)
        target_compile_options(${_premap} PRIVATE ${_arm_opts})
    endif()
    if(_arm_link_opts)
        # The premap must resolve/retain exactly the same host code as the
        # final executable.  In particular, --wrap options can change which
        # functions survive --gc-sections and move the page-aligned CPU1 image.
        target_link_options(${_premap} PRIVATE ${_arm_link_opts})
    endif()
    ...
    # Fail the build if the premap-derived RISC-V link address ever differs
    # from the address where the final ARM link embeds that image. A mismatch
    # is syntactically valid but boots CPU1 in the middle of its vector table.
    add_custom_command(TARGET ${ARM_TARGET} POST_BUILD
        COMMAND ${CMAKE_COMMAND}
            -DARM_NM=${CMAKE_NM}
            -DRISCV_NM=${RISCV_NM}
            -DARM_ELF=$<TARGET_FILE:${ARM_TARGET}>
            -DRISCV_ELF=${_rv_elf}
            -P ${CMAKE_CURRENT_FUNCTION_LIST_DIR}/verify_coprocessor_layout.cmake
        COMMENT "coprocessor: verifying final embed/link address"
        VERBATIM)
endfunction()
