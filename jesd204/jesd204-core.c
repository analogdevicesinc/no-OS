			 ...
			 const struct jesd204_dev_data *i)
{
	...
	dev = (struct jesd204_dev *)malloc(sizeof(*dev));
	if (!dev)
		return -1;

	dev->priv = (void *)malloc(i->sizeof_priv);
...
/* no-OS specific */
int jesd204_init_topology(struct jesd204_topology **topology,
			  ...
			  unsigned int devs_number)
{
	...
	top = (struct jesd204_topology *)malloc(sizeof(*top));
	if (!top)
		return -1;

	top->dev_top = (struct jesd204_dev_top *)malloc(sizeof(*top->dev_top));
	if (!top->dev_top)
		return -1;

	top->devs_number = devs_number - 1;
	top->devs = (struct jesd204_topology_dev *)malloc(top->devs_number * sizeof(*top->devs));
