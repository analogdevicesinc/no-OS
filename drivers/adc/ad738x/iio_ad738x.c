		...
		struct scan_type *scan_type)
{
	...
	if (storage_bits >= AD738X_ADC_RESOLUTION)
		scan_type->shift = storage_bits - AD738X_ADC_RESOLUTION;
...
#if !defined(USE_STANDARD_SPI)
	if (dev->ch_seq_sel == ALL)
		read_samples *= 2;
#endif

	ret = ad738x_read_data(dev, buff, read_samples);
	if (ret)
		return ret;
		    ...
		    struct ad738x_init_param *init_param)
{
	...
	iio_num_channels = num_channels;
#if !defined(USE_STANDARD_SPI)
	if (desc->ad738x_dev->ch_seq_sel == ALL)
		iio_num_channels *= 2;
#endif
	...
	channels = ad738x_iio_alloc_channels(iio_num_channels, storage_bits,
	...
	iio_dev->num_ch = iio_num_channels;
