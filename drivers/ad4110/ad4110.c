...
int32_t ad4110_spi_do_soft_reset(ad4110_dev *dev)
{
	...
	// The AD4110 can be reset by writing a series of 64 1’s to the DIN input
				 ...
				 uint32_t reg_data)
{
	...
	if(((dev->afe_crc_en != AD4110_AFE_CRC_DISABLE) && (reg_map == A4110_AFE))||
	    ((dev->adc_crc_en != AD4110_ADC_CRC_DISABLE) && (reg_map == A4110_ADC))) {
				...
				uint32_t *reg_data)
{
	...
	*reg_data = data;
	...
	if(((dev->afe_crc_en == AD4110_AFE_CRC) && (reg_map == A4110_AFE))||
	    ((dev->adc_crc_en == AD4110_ADC_CRC_CRC) && (reg_map == A4110_ADC))) {
	...
	} else if ((dev->adc_crc_en == AD4110_ADC_XOR_CRC) && (reg_map == A4110_ADC)) {
		buf[0] = (reg_map << 7) | AD4110_CMD_READ_COM_REG(reg_addr);
		     ...
		     ad4110_init_param init_param)
{
	ad4110_dev *dev;
	int32_t ret;
	dev = (ad4110_dev *)malloc(sizeof(*dev));
	if (!dev) {
		return -1;
	}
