				  ...
				  intptr_t priv)
{
	...
	case MAXM86161_IIO_SAMPLE_RATE:
		ret = maxm86161_get_sample_rate(dev, &val8);
		if (ret)
			return ret;
		return snprintf(buf, len, "%u", val8);

	case MAXM86161_IIO_INTEGRATION_TIME:
		ret = maxm86161_get_integration_time(dev, &tint);
		if (ret)
			return ret;
		return snprintf(buf, len, "%u", tint);

	case MAXM86161_IIO_ADC_RANGE:
		ret = maxm86161_get_adc_range(dev, &adc_range);
		if (ret)
			return ret;
		return snprintf(buf, len, "%u", adc_range);

	case MAXM86161_IIO_SAMPLE_AVERAGING:
		ret = maxm86161_get_sample_averaging(dev, &avg);
		if (ret)
			return ret;
		return snprintf(buf, len, "%u", avg);
	...
	case MAXM86161_IIO_FIFO_WATERMARK:
		ret = maxm86161_get_fifo_watermark(dev, &val8);
		if (ret)
			return ret;
		return snprintf(buf, len, "%u", val8);
	...
	case MAXM86161_IIO_FIFO_OVERFLOW_COUNT:
		return snprintf(buf, len, "%lu",
				(unsigned long)iio_dev->fifo_ovf_count);
	...
	case MAXM86161_IIO_LED3_PA:
		ret = maxm86161_get_led_pulse_amplitude(dev,
							(attr - MAXM86161_IIO_LED1_PA) + 1,
							&val8);
		if (ret)
			return ret;
		return snprintf(buf, len, "%u", val8);
	...
	case MAXM86161_IIO_PF_ENABLE:
		ret = maxm86161_get_pf_enable(dev, &val_bool);
		if (ret)
			return ret;
		return snprintf(buf, len, "%d", val_bool);

	case MAXM86161_IIO_PF_ORDER:
		ret = maxm86161_get_pf_order(dev, &val_bool);
		if (ret)
			return ret;
		return snprintf(buf, len, "%d", val_bool);

	case MAXM86161_IIO_PF_IIR_TC:
		ret = maxm86161_get_pf_iir_tc(dev, &val8);
		if (ret)
			return ret;
		return snprintf(buf, len, "%u", val8);

	case MAXM86161_IIO_PF_IIR_INIT:
		ret = maxm86161_get_pf_iir_init(dev, &val8);
		if (ret)
			return ret;
		return snprintf(buf, len, "%u", val8);

	case MAXM86161_IIO_PF_THRESHOLD_SIGMA:
		ret = maxm86161_get_pf_threshold_sigma(dev, &val8);
		if (ret)
			return ret;
		return snprintf(buf, len, "%u", val8);
	...
	case MAXM86161_IIO_PART_ID:
		return snprintf(buf, len, "0x%02X", MAXM86161_PART_ID_VAL);
				  ...
				  intptr_t priv)
{
	...
	case MAXM86161_IIO_LED1_PA:
		return maxm86161_set_led_pulse_amplitude(dev, MAXM86161_LED_NUM_GREEN,
				(uint8_t)val);

	case MAXM86161_IIO_LED2_PA:
		return maxm86161_set_led_pulse_amplitude(dev, MAXM86161_LED_NUM_IR,
				(uint8_t)val);

	case MAXM86161_IIO_LED3_PA:
		return maxm86161_set_led_pulse_amplitude(dev, MAXM86161_LED_NUM_RED,
				(uint8_t)val);

	case MAXM86161_IIO_LED_PILOT_PA:
		return maxm86161_set_led_pulse_amplitude(dev, MAXM86161_LED_NUM_PILOT_GREEN,
				(uint8_t)val);
	...
	case MAXM86161_IIO_PF_ENABLE:
		return maxm86161_set_pf_enable(dev, (bool)val);

	case MAXM86161_IIO_PF_ORDER:
		return maxm86161_set_pf_order(dev, (bool)val);

	case MAXM86161_IIO_PF_IIR_TC:
		return maxm86161_set_pf_iir_tc(dev, (uint8_t)val);

	case MAXM86161_IIO_PF_IIR_INIT:
		return maxm86161_set_pf_iir_init(dev, (uint8_t)val);

	case MAXM86161_IIO_PF_THRESHOLD_SIGMA:
		return maxm86161_set_pf_threshold_sigma(dev, (uint8_t)val);
	...
	case MAXM86161_IIO_BURST_EN:
		return maxm86161_set_burst_enable(dev, (bool)val);

	case MAXM86161_IIO_BURST_RATE:
		return maxm86161_set_burst_rate(dev, (enum maxm86161_burst_rate)val);
...
static int maxm86161_iio_reg_read(void *device, uint32_t reg, uint32_t *val)
{
	struct maxm86161_iio_desc *iio_dev = device;
	uint8_t reg_val;
	int ret;
...
static int maxm86161_iio_reg_write(void *device, uint32_t reg, uint32_t val)
{
	struct maxm86161_iio_desc *iio_dev = device;
...
static int maxm86161_iio_submit(struct iio_device_data *dev_data)
{
...
void maxm86161_iio_fifo_irq_handler(void *ctx)
{
	...
	if (overflow_count > 0) {
		/*
		 * The device FIFO wrapped before this handler could drain it, so
		 * @p overflow_count samples were lost on the hardware side. Track it
		 * for the consumer to poll via the fifo_overflow_count attribute.
		 * Kept silent (no logging) because this runs in interrupt context.
		 */
		iio_dev->fifo_ovf_count += overflow_count;
	}
...
static struct iio_attribute maxm86161_debug_attrs[] = {
	...
	MAXM86161_ATTR_RO("fifo_overflow_count", MAXM86161_IIO_FIFO_OVERFLOW_COUNT),
	...
	MAXM86161_ATTR("picket_fence_iir_tc", MAXM86161_IIO_PF_IIR_TC),
	MAXM86161_ATTR("picket_fence_iir_init", MAXM86161_IIO_PF_IIR_INIT),
	MAXM86161_ATTR("picket_fence_threshold_sigma",
		       MAXM86161_IIO_PF_THRESHOLD_SIGMA),
