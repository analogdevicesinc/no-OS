...
{
	int ret = 0;
	/* max size of applicable burst write op */
	uint8_t buf[MAXM86161_SHA_CHALLENGE_LEN + 1];

	if (!dev || !reg_data || count > MAXM86161_SHA_CHALLENGE_LEN)
		return -EINVAL;
...
{
	if (!dev || bias > MAXM86161_PD_BIAS_260_520PF)
		return -EINVAL;
			       ...
			       enum maxm86161_led_src src)
{
	...
	if (!dev || slot == 0 || slot > MAXM86161_NUM_LED_SEQ
	    || src > MAXM86161_LED_SRC_DIRECT_AMB)
		return -EINVAL;
...
{
	if (!dev || iir_tc > MAXM86161_PF_IIR_TC_1_8)
		return -EINVAL;
...
{
	if (!dev || iir_init_value > MAXM86161_PF_IIR_INIT_24)
		return -EINVAL;
...
{
	if (!dev || threshold_sigma_mult > MAXM86161_PF_TH_GAIN_32)
		return -EINVAL;
...
{
	if (!dev || ctrl > MAXM86161_GPIO_SAMPLE_SYNC_ONESHOT)
		return -EINVAL;
