...
* **max20362_set_dvs_rr_table()**: Programs the Round-Robin voltage
  sequence table.
* **max20362_set_rr_wrap()**: Configures whether the round-robin pointer
  wraps back to the first entry or stays on the last entry after reaching
  the end of the table.
* **max20362_set_rr_clk_polarity()**: Selects the CLKIN edge that advances
  the round-robin pointer (rising or falling edge).
* **max20362_set_rr_timeout_reset()**: Configures whether the round-robin
  pointer resets to the beginning after a timeout event.
...
1. Configure the RRWrap behavior using **max20362_set_rr_wrap()** to
   determine whether the pointer wraps back to the first entry or remains
   on the last entry after reaching the end of the table.
2. Configure the clock polarity using **max20362_set_rr_clk_polarity()** to
   select rising or falling edge triggering on the CLKIN signal.
3. Configure the timeout reset behavior using
   **max20362_set_rr_timeout_reset()** to determine whether the round-robin
   pointer resets after a timeout.
4. Program up to 20 DVS voltage values using **max20362_set_dvs_rr_table()**.
   This writes the voltages into the round-robin table, configures the table
   size (RRSize), and enables round-robin mode.

.. note::

   RRWrap, RRClkPolarity, RRTimeoutRes, and RRSize should not be changed
   while round-robin mode is enabled. Configure these settings before
   enabling round-robin mode or disable it first before reconfiguring.
