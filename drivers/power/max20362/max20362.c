		  ...
		  const struct max20362_init_param *init_param)
{
	...
	/* Configure DVS source */
	ret = max20362_set_dvs_mode(dev, init_param->dvs_source);
	if (ret)
		goto error;
	dev->dvs_source = init_param->dvs_source;
			      ...
			      const uint32_t *voltages_uv, uint8_t count)
{
	...
	/* DVS must NOT be in round-robin mode to program RR table */
	if (dev->dvs_source == MAX20362_DVS_SOURCE_ROUND_ROBIN)
		return -EINVAL;
	...
	ret = max20362_reg_update_bits(dev, MAX20362_REG_DVS_CFG,
				       MAX20362_DVSCFG_RRENA_MSK, MAX20362_DVSCFG_RRENA_MSK);
	if (!ret)
		dev->dvs_source = MAX20362_DVS_SOURCE_ROUND_ROBIN;

	return ret;
}

/**
 * @brief Set round-robin wrap mode
 * @param dev - Device structure
 * @param enable - true: wrap back to first entry, false: stay on last entry
 * @return 0 on success, negative error code otherwise
 */
int max20362_set_rr_wrap(struct max20362_dev *dev, bool enable)
{
	if (!dev)
		return -ENODEV;

	return max20362_reg_update_bits(dev, MAX20362_REG_DVS_CFG,
					MAX20362_DVSCFG_RRWRAP_MSK,
					enable ? MAX20362_DVSCFG_RRWRAP_MSK : 0);
}

/**
 * @brief Set round-robin clock polarity
 * @param dev - Device structure
 * @param rising_edge - true: rising edge active, false: falling edge active
 * @return 0 on success, negative error code otherwise
 */
int max20362_set_rr_clk_polarity(struct max20362_dev *dev, bool rising_edge)
{
	if (!dev)
		return -ENODEV;
	...
	return max20362_reg_update_bits(dev, MAX20362_REG_RR_CFG1,
					MAX20362_RRCFG1_RRCLKPOLARITY_MSK,
					rising_edge ? MAX20362_RRCFG1_RRCLKPOLARITY_MSK : 0);
}

/**
 * @brief Set round-robin timeout reset behavior
 * @param dev - Device structure
 * @param enable - true: timeout resets RR pointer, false: timeout does not reset
 * @return 0 on success, negative error code otherwise
 */
int max20362_set_rr_timeout_reset(struct max20362_dev *dev, bool enable)
{
	if (!dev)
		return -ENODEV;
	...
	return max20362_reg_update_bits(dev, MAX20362_REG_RR_CFG1,
					MAX20362_RRCFG1_RRTIMEOUTRES_MSK,
					enable ? MAX20362_RRCFG1_RRTIMEOUTRES_MSK : 0);
}
