				  ...
				  const struct capi_timer_config *config)
{
	...
	if (alloc_handle) {
		*handle = capi_malloc(sizeof(struct capi_timer_xilinx_handle));
		if (!*handle)
			return -ENOMEM;
	}

	struct capi_timer_xilinx_handle *xh = (struct capi_timer_xilinx_handle *)(
			*handle);
	memset(xh, 0, sizeof(*xh));
	xh->handle.init_allocated = alloc_handle;
	xh->handle.ops = config->ops ? config->ops : &capi_timer_xilinx_ps_scu_ops;
	...
	if (xcfg != NULL && xcfg->use_irq) {
		...
		int ret = capi_irq_connect(irq_id, irq_handler, xh);
		if (ret == 0) {
...
static int capi_timer_ps_scu_deinit(struct capi_timer_handle *handle)
{
	...
	struct capi_timer_xilinx_handle *xh = (struct capi_timer_xilinx_handle *)handle;
...
static int capi_timer_ps_scu_start(struct capi_timer_handle *handle)
{
	...
	XScuTimer_Start(inst((struct capi_timer_xilinx_handle *)handle));
...
static int capi_timer_ps_scu_stop(struct capi_timer_handle *handle)
{
	...
	XScuTimer_Stop(inst((struct capi_timer_xilinx_handle *)handle));
		...
		uint64_t duration_ns, uint32_t *ticks)
{
	...
	const struct capi_timer_xilinx_handle *xh = (const struct
			capi_timer_xilinx_handle *)handle;

	if (xh->input_clock_hz == 0)
		return -EINVAL;

	uint64_t whole = duration_ns / 1000000000ULL;
	uint64_t remainder = duration_ns % 1000000000ULL;

	if (whole > UINT32_MAX / xh->input_clock_hz)
		return -EOVERFLOW;

	uint64_t result = whole * xh->input_clock_hz;
	uint64_t fraction = (remainder * xh->input_clock_hz) / 1000000000ULL;
		...
		uint32_t *duration_ns)
{
	...
	if (whole > UINT32_MAX / 1000000000ULL)
		return -EOVERFLOW;

	uint64_t result = whole * 1000000000ULL;
	uint64_t fraction = (remainder * 1000000000ULL) / xh->input_clock_hz;
