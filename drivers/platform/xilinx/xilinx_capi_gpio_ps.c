                                      ...
                                      const struct capi_gpio_port_config *config)
{
	...
	if (alloc_handle) {
		*handle = malloc(sizeof(struct capi_gpio_xilinx_port_handle));
		if (*handle == NULL)
			return -ENOMEM;
	}
	struct capi_gpio_xilinx_port_handle *xhandle =
	        (struct capi_gpio_xilinx_port_handle *)(*handle);
	memset(xhandle, 0, sizeof(*xhandle));
	xhandle->handle.init_allocated = alloc_handle;
	xhandle->handle.ops = config->ops;
	xhandle->base_pin = base_pin;
	xhandle->num_pins = config->num_pins;

	xhandle->pin_flags = malloc(config->num_pins * sizeof(uint32_t));
	if (xhandle->pin_flags == NULL) {
		if (alloc_handle)
			free(xhandle);
		return -ENOMEM;
	...
	if (ps_cfg == NULL) {
		free(xhandle->pin_flags);
		if (alloc_handle)
			free(xhandle);
		return -ENODEV;
	}

	XGpioPs *ps_inst = malloc(sizeof(XGpioPs));
	if (ps_inst == NULL) {
		free(xhandle->pin_flags);
		if (alloc_handle)
			free(xhandle);
...
// The only difference is that Versal function requires Instance Pointer
#ifdef versal
	...
	*handle = (struct capi_gpio_port_handle *)xhandle;
	return 0;
}
...
static int capi_gpio_xilinx_port_deinit(struct capi_gpio_port_handle **handle)
{
	...
	if (xhandle->handle.init_allocated)
		free(xhandle);
...
{
	struct capi_gpio_xilinx_port_handle *xhandle =
	        (struct capi_gpio_xilinx_port_handle *)pin->port_handle;
