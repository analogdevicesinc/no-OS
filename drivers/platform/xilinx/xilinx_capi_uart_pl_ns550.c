                                ...
                                const struct capi_uart_config *config)
{
	...
	if (alloc) {
		*handle = malloc(sizeof(struct capi_uart_xilinx_handle));
		if (*handle == NULL)
			return -ENOMEM;
	}
	...
	xh->handle.ops = config->ops ? config->ops : &capi_uart_xilinx_pl_ns550_ops;

	xh->line_config.baudrate = 115200;
	...
	XUartNs550 *ns = malloc(sizeof(XUartNs550));
	if (ns == NULL) {
		if (alloc) {
			free(xh);
			*handle = NULL;
	...
	capi_uart_ns550_set_line_config((struct capi_uart_handle *)xh,
	                                &xh->line_config);
	...
	if (xcfg != NULL && xcfg->use_irq) {
		...
		int ret = capi_irq_connect(irq_id, capi_uart_ns550_isr, xh);
		if (ret == 0) {
	...
	*handle = (struct capi_uart_handle *)xh;
	return 0;
}
...
static int capi_uart_ns550_deinit(struct capi_uart_handle *handle)
{
	...
	struct capi_uart_xilinx_handle *xh = (struct capi_uart_xilinx_handle *)handle;
                                       ...
                                       bool enable)
{
	...
	if (enable)
		...
		XUartNs550_WriteReg(base, XUN_FCR_OFFSET, 0x00);
