...
static u8 spips_calc_prescaler(u32 input_clk_hz, u32 desired_hz)
{
	...
	for (prescale = XSPIPS_CLK_PRESCALE_4; prescale <= XSPIPS_CLK_PRESCALE_256;
	     prescale++) {
		u32 actual_hz = input_clk_hz / (1U << (prescale + 1));
...
static int spips_set_frequency(XSpiPs *spi, u32 desired_hz)
{
	...
	u32 actual_hz = spi->Config.InputClockHz / (1U << (prescale + 1));
	if (actual_hz > desired_hz)
		return -ENOTSUP;

	return (XSpiPs_SetClkPrescaler(spi, prescale) == XST_SUCCESS) ? 0 : -EIO;
}

/* Convert a one-hot native_cs bitmask to PS SPI slave index (0, 1, or 2). */
static int ps_cs_to_index(uint16_t native_cs, u8 *index)
{
	uint16_t mask = native_cs ? native_cs : 1U;
	if (index == NULL || mask > 0x04U || (mask & (mask - 1U)) != 0U)
		return -EINVAL;
                            ...
                            const struct capi_spi_device *device)
{
	...
	return (XSpiPs_SetSlaveSelect(inst(xh), index) == XST_SUCCESS) ? 0 : -EIO;
}
                              ...
                              uint16_t native_cs, bool assert)
{
	...
	if (ret != 0)
		return ret;
	u8 index = assert ? selected : 0x0FU;
	if (XSpiPs_SetSlaveSelect(inst(xh), index) != XST_SUCCESS)
		return -EIO;
                            ...
                            const struct capi_spi_config *config)
{
	...
	xh->handle.ops = config->ops ? config->ops : &capi_spi_xilinx_ps_ops;
	...
	XSpiPs *ps = malloc(sizeof(XSpiPs));
	if (ps == NULL) {
		if (alloc) {
			free(xh);
			*handle = NULL;
	...
	if (XSpiPs_CfgInitialize(ps, cfg, cfg->BaseAddress) != XST_SUCCESS) {
		free(ps);
		if (alloc) {
			free(xh);
			*handle = NULL;
	...
	if (XSpiPs_SetOptions(ps, options) != XST_SUCCESS) {
		free(ps);
		xh->instance = NULL;
		if (alloc) {
			free(xh);
	...
	if (config->clk_freq_hz == 0) {
		ret = (XSpiPs_SetClkPrescaler(ps, XSPIPS_CLK_PRESCALE_64) ==
		       XST_SUCCESS) ? 0 : -EIO;
	} else {
	...
	if (xcfg != NULL && xcfg->use_irq) {
		...
		ret = capi_irq_connect(irq_id, capi_spi_ps_isr, xh);
		if (ret == 0) {
                                  ...
                                  struct capi_spi_transfer *transfer)
{
	...
	if (XSpiPs_SetOptions(inst(xh), options) != XST_SUCCESS) {
		free(tx);
		if (need_rx_temp)
			free(rx);
		return -EIO;
	...
	if (use_gpio_cs && device->non_continuous_mode) {
		...
		for (uint16_t i = 0; i < len && status == XST_SUCCESS; i++) {
			...
			status = XSpiPs_PolledTransfer(inst(xh), tx + i, rx ? rx + i : NULL, 1);
	...
	if (!use_gpio_cs)
		XSpiPs_SetSlaveSelect(inst(xh), 0x0F);
                                        ...
                                        struct capi_spi_transfer *transfer, int timeout)
{
	...
	if (XSpiPs_Transfer(inst(xh), tx, rx, len) != XST_SUCCESS) {
		(void)ps_async_cleanup(xh, false);
		return -EIO;
	}
                ...
                struct capi_spi_transfer *transfer)
{
	...
	if (XSpiPs_Transfer(inst(xh), tx, rx, total) != XST_SUCCESS) {
		(void)ps_async_cleanup(xh, false);
		return -EIO;
	}
