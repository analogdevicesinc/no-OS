				  ...
				  const struct capi_uart_config *config)
{
	...
	xh->handle.ops = config->ops ? config->ops : &capi_uart_xilinx_pl_lite_ops;

	/* Fixed 8N1; cache what hardware is set to. */
	xh->line_config.baudrate =
		9600; /* synthesis-time default, unknown at runtime */
	...
	XUartLite *ul = malloc(sizeof(XUartLite));
	if (ul == NULL) {
		if (alloc) {
			free(xh);
			*handle = NULL;
	...
	if (xcfg != NULL && xcfg->use_irq) {
		...
		int ret = capi_irq_connect(irq_id, capi_uart_pl_lite_isr, xh);
		if (ret == 0) {
