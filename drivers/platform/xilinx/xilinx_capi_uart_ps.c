			     ...
			     const struct capi_uart_config *config)
{
	...
	xh->handle.ops = config->ops ? config->ops : &capi_uart_xilinx_ps_ops;
	...
	XUartPs *ps = malloc(sizeof(XUartPs));
	if (ps == NULL) {
		if (alloc) {
			free(xh);
			*handle = NULL;
	...
	capi_uart_ps_set_line_config((struct capi_uart_handle *)xh, &xh->line_config);
	...
	if (xcfg != NULL && xcfg->use_irq) {
		...
		int ret = capi_irq_connect(irq_id, capi_uart_ps_isr, xh);
		if (ret == 0) {
