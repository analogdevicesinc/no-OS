...
static int _max_spi_config(struct no_os_spi_desc *desc)
{
	...
	spi_pin_init.vssel = true; //TODO: Update this value based on ini param
	spi_pin_init.map_a = true;

	ret = MXC_SPI_Init(MXC_SPI_GET_SPI(desc->device_id), SPI_MASTER_MODE,
			   SPI_SINGLE_MODE,
			   eparam->num_slaves, eparam->polarity, desc->max_speed_hz, spi_pin_init);
	if (ret) {
		ret = -EINVAL;
		goto err_init;
	}
					...
					void *ctx, bool is_async)
{
	...
	spi->dma |= MXC_F_SPI_DMA_RX_EN | MXC_F_SPI_DMA_TX_EN;
