...
static GPIO_TypeDef *get_gpio_port_from_identifier(uint64_t identifier)
{
	...
	if (identifier >= 0x100)
		return (GPIO_TypeDef *)identifier;
...
static int enable_gpio_port_clock(uint64_t identifier)
{
	...
	if (identifier >= 0x40000000) {
		GPIO_TypeDef *port = (GPIO_TypeDef *)identifier;
			      ...
			      const struct capi_gpio_port_config *config)
{
	...
	if (*handle == NULL) {
		port_handle = no_os_calloc(1, sizeof(*port_handle));
		if (!port_handle)
			return -ENOMEM;
	...
	priv->direction_mask = 0xFFFF; /* All inputs by default */

	/* Apply extra configuration if provided */
	if (config->priv) {
		extra_config = (struct stm32_capi_gpio_port_config *)config->priv;
...
int stm32_capi_gpio_port_deinit(struct capi_gpio_port_handle **handle)
{
	...
	/* Free private data */
	if (port_handle->priv)
		no_os_free(port_handle->priv);

	/* Free handle if we allocated it */
	if (port_handle->init_allocated)
		no_os_free(port_handle);
				       ...
				       uint64_t value_bitmask)
{
	...
	priv->port->BSRR = ((uint32_t)reset_pins << 16) | set_pins;
				       ...
				       uint64_t *value_bitmask)
{
	...
	*value_bitmask = priv->port->IDR & 0xFFFF;
...
int stm32_capi_gpio_pin_set_direction(struct capi_gpio_pin *pin, uint8_t direction)
{
	...
	if (pin->number < 0 || pin->number >= STM32_GPIO_PINS_PER_PORT)
		return -EINVAL;

	priv = pin->port_handle->priv;

	gpio_config.Pin = NO_OS_BIT(pin->number);
	gpio_config.Pull = priv->default_config.pull;
	gpio_config.Speed = priv->default_config.speed;

	if (direction == CAPI_GPIO_INPUT) {
		gpio_config.Mode = GPIO_MODE_INPUT;
		priv->direction_mask |= NO_OS_BIT(pin->number);
	} else {
		/* Read current pin state and set output value before configuring
		 * direction to avoid glitches on the pin */
		current_value = (priv->port->IDR & NO_OS_BIT(pin->number)) ? 1 : 0;
		HAL_GPIO_WritePin(priv->port, NO_OS_BIT(pin->number),
...
int stm32_capi_gpio_pin_get_direction(struct capi_gpio_pin *pin, uint8_t *direction)
{
	...
	if (priv->direction_mask & NO_OS_BIT(pin->number))
...
int stm32_capi_gpio_pin_set_raw_value(struct capi_gpio_pin *pin, uint8_t value)
{
	...
	if (value)
		priv->port->BSRR = NO_OS_BIT(pin->number);
	else
		priv->port->BSRR = NO_OS_BIT(pin->number) << 16;
...
int stm32_capi_gpio_pin_get_raw_value(struct capi_gpio_pin *pin, uint8_t *value)
{
	...
	*value = (priv->port->IDR & NO_OS_BIT(pin->number)) ? CAPI_GPIO_HIGH : CAPI_GPIO_LOW;
