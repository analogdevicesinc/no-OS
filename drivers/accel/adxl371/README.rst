...
``adxl371_write_mask`` for manipulating register values.
...
.. code-block:: C
   ...
   int main(void)
   {
   	...
   	ret = adxl371_init(&adxl371_desc, adxl_ip);
   	if (ret)
   		goto cleanup;

   	ret = adxl371_set_op_mode(adxl371_desc, ADXL371_STANDBY);
   	if (ret)
   		goto cleanup;

   	/** NOTE: Workaround for er001 requires internal sync to start */
   	adxl371_set_internal_sync(adxl371_desc);

   	adxl371_set_op_mode(adxl371_desc, ADXL371_FULL_BW_MEASUREMENT);

   	/* Read # available samples into "fifo_entries" */
   	ret = adxl371_get_status(adxl371_desc, &status1, &status2, &fifo_entries);
   	if (ret)
   		goto cleanup;

   	/* Read raw acceleration data */
   	ret = adxl371_get_fifo_xyz_data_workaround(adxl371_desc, accel_data,
   			fifo_entries);
   	if (ret)
   		goto cleanup;
...
.. code-block:: C
   	...
   	/* Samples saved after the activity threshold for full event capture mode.
   	 * Rules
   	 *  - must be < 512 samples
   	 *  - should be multiples of 3 for 3-axis sampling
   	 *  - account for one full 3-axis sample left in FIFO for ordering.
   	 *    (so add 3 if using 3 axis samples)
	 */
   	#define EVENTS_TRAILING_SAMPLES 270
   	...
   	void adxl_configure_impact(struct adxl371_dev *dev)
   	{
   		...
   		err = adxl371_get_status(dev, &status1, &status2, &fifo_entries);
   		if (err) {
   			printf("ERR Getting Status %d\n", err);
   			while(1);
   		}
   		adxl371_get_fifo_xyz_data_workaround(dev, accel_data, fifo_entries);

   		/* Clear all status flags */
   		adxl371_get_status(dev, &status1, &status2, &fifo_entries);
