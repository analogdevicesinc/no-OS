/*******************************************************************************
 *   @file   adxl37x_i2c.c
 *   @brief  Implementation of adxl37x I2C Driver.
 *   @author BHurst (brandon.hurst@analog.com)
 ********************************************************************************
 * Copyright 2026(c) Analog Devices, Inc.
 *
 * SPDX-License-Identifier: BSD-3-Clause
*******************************************************************************/

#include "adxl37x.h"
#include "no_os_error.h"
#include <stdbool.h>
#include <stdlib.h>
#include <string.h>

/* Largest multibyte I2C transfer the driver will service, in bytes. */
#define ADXL37X_I2C_MAX_XFER_BYTES	512

/**
 * Read from device.
 * @param dev - The device structure.
 * @param reg_addr - The register address.
 * @param reg_data - The register data.
 * @return 0 in case of success, negative error code otherwise.
 */
int32_t adxl37x_i2c_reg_read(struct adxl37x_dev *dev, uint8_t reg_addr,
			     uint8_t *reg_data)
{
	uint8_t data;
	int32_t ret;

	if (!dev)
		return -EINVAL;

	data = reg_addr;

	ret = no_os_i2c_write(dev->i2c_desc, &data, 1, 0);
	if (ret)
		return ret;

	ret = no_os_i2c_read(dev->i2c_desc, &data, 1, 0);
	if (ret)
		return ret;

	*reg_data = data;

	return ret;
}

/**
 * Write to device.
 * @param dev - The device structure.
 * @param reg_addr - The register address.
 * @param reg_data - The register data.
 * @return 0 in case of success, negative error code otherwise.
 */
int32_t adxl37x_i2c_reg_write(struct adxl37x_dev *dev, uint8_t reg_addr,
			      uint8_t reg_data)
{
	uint8_t buf[2];

	if (!dev)
		return -EINVAL;

	buf[0] = reg_addr;
	buf[1] = reg_data & 0xFF;

	return no_os_i2c_write(dev->i2c_desc, buf, NO_OS_ARRAY_SIZE(buf), 0);
}

/**
 * Multibyte read from device. A register read begins with the address
 * and autoincrements for each aditional byte in the transfer.
 * @param dev - The device structure.
 * @param reg_addr - The register address.
 * @param reg_data - The register data.
 * @param count - Number of bytes to read.
 * @return 0 in case of success, negative error code otherwise.
 */
int32_t adxl37x_i2c_reg_read_multiple(struct adxl37x_dev *dev, uint8_t reg_addr,
				      uint8_t *reg_data, uint16_t count)
{
	uint8_t buf[ADXL37X_I2C_MAX_XFER_BYTES] = {0};
	int32_t ret;

	if (!dev)
		return -EINVAL;

	if (count > ADXL37X_I2C_MAX_XFER_BYTES)
		return -EINVAL;

	buf[0] = reg_addr;

	ret = no_os_i2c_write(dev->i2c_desc, buf, 1, 0);
	if (ret)
		return ret;

	ret = no_os_i2c_read(dev->i2c_desc, buf, count, 0);
	if (ret)
		return ret;

	memcpy(reg_data, buf, count);

	return ret;
}
