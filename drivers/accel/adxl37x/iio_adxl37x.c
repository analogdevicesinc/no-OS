...
/* Active-channel mask bits, as handed to pre_enable/update_channels. */
#define ACCEL_MASK_X	NO_OS_BIT(ACCEL_AXIS_X)
#define ACCEL_MASK_Y	NO_OS_BIT(ACCEL_AXIS_Y)
#define ACCEL_MASK_Z	NO_OS_BIT(ACCEL_AXIS_Z)
#define ACCEL_MASK_XYZ	(ACCEL_MASK_X | ACCEL_MASK_Y | ACCEL_MASK_Z)

/* Scratch buffer for formatting one value of an *_available list. */
#define ATTR_BUF_SIZE	50
 ...
 */
static const int32_t adxl371_iio_odr_table[] = { 320, 640, 1280, 2560, 5120 };
static const int32_t adxl372_iio_odr_table[] = { 400, 800, 1600, 3200, 6400 };

#define ODR_MODES	NO_OS_ARRAY_SIZE(adxl371_iio_odr_table)
 ...
 */
static const int32_t adxl371_iio_lpf_3db_multipliers[] = { 160, 320, 640, 1280, 2560 };
static const int32_t adxl372_iio_lpf_3db_multipliers[] = { 200, 400, 800, 1600, 3200 };

#define LPF_FREQ	NO_OS_ARRAY_SIZE(adxl371_iio_lpf_3db_multipliers)
...
{
	int32_t ret;
	uint8_t raw_buf[ADXL37X_BYTES_PER_AXIS];
	uint8_t reg;
	int32_t val;
	...
	/* Read only the data register pair of the requested axis. */
	switch (channel->address) {
	case ACCEL_AXIS_X:
		reg = ADXL37X_X_DATA_H;
		break;
	case ACCEL_AXIS_Y:
		reg = ADXL37X_Y_DATA_H;
		break;
	case ACCEL_AXIS_Z:
		reg = ADXL37X_Z_DATA_H;
		break;
	default:
		return -EINVAL;
	}

	ret = adxl37x->reg_read_multiple(adxl37x, reg, raw_buf,
					 ADXL37X_BYTES_PER_AXIS);
	if (ret)
		return ret;

	val = no_os_sign_extend32((raw_buf[0] << ADXL37X_DATA_SHIFT) |
				  (raw_buf[1] >> ADXL37X_DATA_SHIFT),
				  ...
				  const struct iio_ch_info *channel, intptr_t priv)
{
	...
	if (!channel || channel->type == IIO_ACCEL) {
		vals[0] = 0;
				      ...
				      const struct iio_ch_info *channel, intptr_t priv)
{
	...
	switch (channel->address) {
	case ACCEL_AXIS_X:
		reg = ADXL37X_OFFSET_X;
		break;
	case ACCEL_AXIS_Y:
		reg = ADXL37X_OFFSET_Y;
		break;
	case ACCEL_AXIS_Z:
		reg = ADXL37X_OFFSET_Z;
		break;
	default:
	...
	ret = adxl37x_read_reg(adxl37x, reg, &raw_val);
	if (ret)
		return ret;
	 ...
	 */
	val = no_os_sign_extend32(raw_val & ADXL37X_OFFSET_MSK,
				  ADXL37X_OFFSET_SIGN_BIT);
				       ...
				       const struct iio_ch_info *channel, intptr_t priv)
{
	...
	if (channel->type == IIO_ACCEL) {
		...
		if (calibbias < ADXL37X_OFFSET_MIN ||
		    calibbias > ADXL37X_OFFSET_MAX) {
			pr_err("Offset value is a signed value on 4 bits.\r\n");
			return -EINVAL;
		}
		/* Only the lower nibble is needed. */
		calibbias = calibbias & ADXL37X_OFFSET_MSK;
		...
		const struct iio_ch_info *channel, intptr_t priv)
{
	...
	switch (adxl37x->adxl_type) {
	case ADXL371:
		val = adxl371_iio_odr_table[adxl37x->odr];
		break;
	case ADXL372:
		val = adxl372_iio_odr_table[adxl37x->odr];
		break;
	default:
		return -EINVAL;
	}
		...
		const struct iio_ch_info *channel, intptr_t priv)
{
	...
	if (!channel || channel->type == IIO_ACCEL) {
		...
		switch (adxl37x->adxl_type) {
		case ADXL371:
			used_ODR_table = adxl371_iio_odr_table;
			break;
		case ADXL372:
		...
		const struct iio_ch_info *channel, intptr_t priv)
{
	...
	if (!channel || channel->type == IIO_ACCEL) {
		...
		switch (adxl37x->adxl_type) {
		case ADXL371:
			used_freq_table = adxl371_iio_odr_table;
			break;
		case ADXL372:
				     ...
				     const struct iio_ch_info *channel, intptr_t priv)
{
	...
	switch (adxl37x->adxl_type) {
	case ADXL371:
		val = adxl371_iio_lpf_3db_multipliers[adxl37x->bw];
		break;
	case ADXL372:
		val = adxl372_iio_lpf_3db_multipliers[adxl37x->bw];
		break;
	default:
		return -EINVAL;
	}
				      ...
				      const struct iio_ch_info *channel, intptr_t priv)
{
	...
	if (!channel || channel->type == IIO_ACCEL) {
		...
		switch (adxl37x->adxl_type) {
		case ADXL371:
			used_lpf_table = adxl371_iio_lpf_3db_multipliers;
			break;
		case ADXL372:
				       ...
				       enum adxl37x_fifo_format *format)
{
	...
	case ACCEL_MASK_Z:
		*format = ADXL37X_Z_FIFO;
		return 0;
	case (ACCEL_MASK_X | ACCEL_MASK_Y):
		*format = ADXL37X_XY_FIFO;
		return 0;
	case (ACCEL_MASK_X | ACCEL_MASK_Z):
		*format = ADXL37X_XZ_FIFO;
		return 0;
	case (ACCEL_MASK_Y | ACCEL_MASK_Z):
...
static int adxl37x_iio_update_channels(void *dev, uint32_t mask)
{
	...
	if (mask & ACCEL_MASK_X)
		counter++;
	if (mask & ACCEL_MASK_Y)
		counter++;
	if (mask & ACCEL_MASK_Z)
		counter++;
	...
	adxl37x->fifo_config.bytes_per_sample = ADXL37X_BYTES_PER_AXIS * counter;

	switch (counter) {
	case 1:
		max_samples = ADXL37X_FIFO_MAX_ENTRIES;
		break;
	case 2:
		max_samples = ADXL37X_FIFO_MAX_ENTRIES / 2;
		break;
	default:
		max_samples = ADXL37X_FIFO_MAX_SAMPLES;
		break;
	}
...
static int adxl37x_iio_read_samples(void *dev, int *buff, uint32_t samples)
{
	...
	for (s = 0; s < samples; s++) {
		if (iio_adxl37x->active_channels & ACCEL_MASK_X)
			buff[i++] = data_buffer[s].x;
		if (iio_adxl37x->active_channels & ACCEL_MASK_Y)
			buff[i++] = data_buffer[s].y;
		if (iio_adxl37x->active_channels & ACCEL_MASK_Z)
		     ...
		     struct adxl37x_iio_dev_init_param *init_param)
{
	...
	if (!iio_dev || !init_param || !init_param->adxl37x_dev_init)
		return -EINVAL;
