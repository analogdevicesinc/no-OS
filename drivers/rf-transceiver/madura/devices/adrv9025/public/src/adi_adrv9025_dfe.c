                                     ...
                                     adi_adrv9025_DpdStatus_v2_t* dpdStatus)
{
    ...
    int32_t recoveryAction = ADI_COMMON_ACT_NO_ACTION;
    uint8_t extData[3] = { ADRV9025_CPU_OBJECTID_CAL_STATUS, ADRV9025_CPU_OBJECTID_DPD_TRACKING_NO_FLOAT, 0 };
    uint8_t armData[112] = { 0 };
    uint8_t cmdStatusByte = 0;
    ...
    /* Deserializing DPD Status into the structure. */
    adrv9025_DpdStatusDeserialize_v2(device,
                                     dpdStatus,
                                     &armData[0],
                                     sizeof(armData));
                                          ...
                                          adi_adrv9025_DpdModelConfig_v2_t* dpdModelConfig)
{
...
#if ADI_ADRV9025_DPD_RANGE_CHECK > 0
    adrv9025_DpdModelConfigSetRangeCheck_v2(device,
                                            dpdModelConfig);
    ADI_ERROR_RETURN(device->common.error.newAction);
#endif

    /* Encode the DPD model config into a format expected by the ADRV9025 firmware */
    recoveryAction = adrv9025_DpdModelConfigEncode_v2(device,
                                                      dpdModelConfig,
                                                      &armConfigData[0],
                                                      sizeof(armConfigData));
    ADI_ERROR_REPORT(&device->common,
    ...
    recoveryAction = adi_adrv9025_CpuConfigWrite(device,
                                                 ADI_ADRV9025_CPU_TYPE_C,
                                                 ADRV9025_CPU_OBJECTID_DPD_TRACKING_NO_FLOAT,
                                                 0,
                                                 &armConfigData[0],
                                                 ADI_ADRV9025_NUM_BYTES_DPD_MODEL_METADATA);
    ADI_ERROR_REPORT(&device->common,
                                          ...
                                          adi_adrv9025_DpdModelConfig_v2_t* dpdModelConfig)
{
    ...
    /* Decode the serialized DPD model config read from the device */
    recoveryAction = adrv9025_DpdModelConfigDecode_v2(device,
                                                      &armConfigData[0],
                                                      sizeof(armConfigData),
                                                      dpdModelConfig);
    ADI_ERROR_REPORT(&device->common,
                                          ...
                                          adi_adrv9025_DpdTrackingConfig_t* dpdTrackingConfig)
{
    ...
    recoveryAction = adrv9025_dpdTrackingConfigSet(device, dpdTrackingConfig);
                                          ...
                                          adi_adrv9025_DpdTrackingConfig_t* dpdTrackingConfig)
{
    ...
    recoveryAction = adrv9025_dpdTrackingConfigGet(device,txChannel, dpdTrackingConfig);
                                                  ...
                                                  adi_adrv9025_EnhancedDpdTrackingConfig_t* dpdTrackingConfig)
{
    ...
    recoveryAction = adrv9025_dpdEnhancedTrackingConfigSet(device, dpdTrackingConfig);
                                                  ...
                                                  adi_adrv9025_EnhancedDpdTrackingConfig_t* dpdTrackingConfig)
{
    ...
    ADI_NULL_PTR_RETURN(&device->common, dpdTrackingConfig);
    /* call private function with enhanced dpd */
    recoveryAction = adrv9025_dpdEnhancedTrackingConfigGet(device, txChannel, dpdTrackingConfig);
                                                     ...
                                                     adi_adrv9025_EnhancedDpdTrackingConfig_v2_t* dpdTrackingConfig)
{
    ...
    /* call private function with enhanced dpd */
    recoveryAction = adrv9025_dpdEnhancedTrackingConfigSet_v2(device, dpdTrackingConfig);
                                                     ...
                                                     adi_adrv9025_EnhancedDpdTrackingConfig_v2_t* dpdTrackingConfig)
{
    ...
    ADI_NULL_PTR_RETURN(&device->common, dpdTrackingConfig);
    /* call private function with enhanced dpd */
    recoveryAction = adrv9025_dpdEnhancedTrackingConfigGet_v2(device, txChannel, dpdTrackingConfig);
...
int32_t adi_adrv9025_CfrHardClipperConfigSet_v2(adi_adrv9025_Device_t*                 device,
                                                ...
                                                uint8_t                                cfrHardClipperCfgArraySize)
{
    static const uint8_t CFR_CTRL_HCLIP_ENABLE_ID_NO_FLOAT = 22;
    static const uint8_t CFR_HARD_CLIP_ENABLE = 0x01;
    static const uint8_t CFR_HARD_CLIP_DISABLE = 0x00;
...
#if ADI_ADRV9025_CFR_RANGE_CHECK > 0
    adrv9025_CfrHardClipperConfigSetRangeCheck_v2(device,
                                                  cfrHardClipperConfig,
                                                  cfrHardClipperCfgArraySize);
    ADI_ERROR_RETURN(device->common.error.newAction);
#endif
    ...
    for (configIndex = 0; configIndex < cfrHardClipperCfgArraySize; configIndex++)
    {
        armData[0] = (uint8_t)(cfrHardClipperConfig[configIndex].cfrHardClipperThreshold_xM & 0xFF);
        armData[1] = (uint8_t)((cfrHardClipperConfig[configIndex].cfrHardClipperThreshold_xM >> 8) & 0xFF);
        armData[2] = (uint8_t)((cfrHardClipperConfig[configIndex].cfrHardClipperThreshold_xM >> 16) & 0xFF);
        armData[3] = (uint8_t)((cfrHardClipperConfig[configIndex].cfrHardClipperThreshold_xM >> 24) & 0xFF);
        ...
        recoveryAction = adrv9025_CfrCtrlCmdExecute(device,
                                                    cfrHardClipperConfig[configIndex].txChannelMask,
                                                    CFR_CTRL_HCLIP_ENABLE_ID_NO_FLOAT,
                                                    &armData[0],
                                                    sizeof(armData));
        ADI_ERROR_REPORT(&device->common,
                         ADI_COMMON_ERRSRC_API,
                                                ...
                                                adi_adrv9025_CfrHardClipperConfig_v2_t* cfrHardClipperConfig)
{
    ...
    int32_t recoveryAction = ADI_COMMON_ACT_NO_ACTION;
    uint8_t extData[3] = { ADRV9025_CPU_OBJECTID_CAL_STATUS, ADRV9025_CPU_OBJECTID_CFR_INIT_NO_FLOAT, 0 };
    uint8_t armData[5] = { 0 };
    uint8_t cmdStatusByte = 0;
    ...
    cfrHardClipperConfig->cfrHardClipperThreshold_xM = (uint32_t)((uint32_t)armData[0] |
                                                      (uint32_t)armData[1] << 8 |
                                                      (uint32_t)armData[2] << 16 |
                                                      (uint32_t)armData[3] << 24);
    cfrHardClipperConfig->cfrHardClipperEnable = armData[4];
...
{
    static const uint8_t CFR_PULSE_MODE_PROG_ID = 0;
    static const uint8_t CFR_TARGET_THRESH_ID_NO_FLOAT = 21;
    static const uint8_t CFR_INTERPOLATION_ID = 3;
    static const uint8_t CFR_CTRL_TX_DELAY_ID = 7;
    static const uint8_t CFR_MAX_NUM_OF_PEAKS_ID = 10;
    static const uint8_t PULSE_MODE_1A_SELECTION = 3;
...
#if ADI_ADRV9025_CFR_RANGE_CHECK > 0
    adrv9025_CfrCtrlConfigSetRangeCheck_v2(device,
                                           cfrCtrlConfig,
                                           cfrCtrlCfgArraySize);
    ADI_ERROR_RETURN(device->common.error.newAction);
#endif
    ...
    for (cfrCtrlCfgIndex = 0; cfrCtrlCfgIndex < cfrCtrlCfgArraySize; cfrCtrlCfgIndex++)
    {
        ...
        /* Program the CFR Peak threshold and scalars */
        armData[0] = (uint8_t)(cfrCtrlConfig[cfrCtrlCfgIndex].cfrPeakThreshold_xM & 0xFF);
        armData[1] = (uint8_t)((cfrCtrlConfig[cfrCtrlCfgIndex].cfrPeakThreshold_xM >> 8) & 0xFF);
        armData[2] = (uint8_t)((cfrCtrlConfig[cfrCtrlCfgIndex].cfrPeakThreshold_xM >> 16) & 0xFF);
        armData[3] = (uint8_t)((cfrCtrlConfig[cfrCtrlCfgIndex].cfrPeakThreshold_xM >> 24) & 0xFF);

        armData[4] = (uint8_t)(cfrCtrlConfig[cfrCtrlCfgIndex].cfrEngine1PeakThresholdScaler_xM & 0xFF);
        armData[5] = (uint8_t)((cfrCtrlConfig[cfrCtrlCfgIndex].cfrEngine1PeakThresholdScaler_xM >> 8) & 0xFF);
        armData[6] = (uint8_t)((cfrCtrlConfig[cfrCtrlCfgIndex].cfrEngine1PeakThresholdScaler_xM >> 16) & 0xFF);
        armData[7] = (uint8_t)((cfrCtrlConfig[cfrCtrlCfgIndex].cfrEngine1PeakThresholdScaler_xM >> 24) & 0xFF);

        armData[8] = (uint8_t)(cfrCtrlConfig[cfrCtrlCfgIndex].cfrEngine2PeakThresholdScaler_xM & 0xFF);
        armData[9] = (uint8_t)((cfrCtrlConfig[cfrCtrlCfgIndex].cfrEngine2PeakThresholdScaler_xM >> 8) & 0xFF);
        armData[10] = (uint8_t)((cfrCtrlConfig[cfrCtrlCfgIndex].cfrEngine2PeakThresholdScaler_xM >> 16) & 0xFF);
        armData[11] = (uint8_t)((cfrCtrlConfig[cfrCtrlCfgIndex].cfrEngine2PeakThresholdScaler_xM >> 24) & 0xFF);

        armData[12] = (uint8_t)(cfrCtrlConfig[cfrCtrlCfgIndex].cfrEngine3PeakThresholdScaler_xM & 0xFF);
        armData[13] = (uint8_t)((cfrCtrlConfig[cfrCtrlCfgIndex].cfrEngine3PeakThresholdScaler_xM >> 8) & 0xFF);
        armData[14] = (uint8_t)((cfrCtrlConfig[cfrCtrlCfgIndex].cfrEngine3PeakThresholdScaler_xM >> 16) & 0xFF);
        armData[15] = (uint8_t)((cfrCtrlConfig[cfrCtrlCfgIndex].cfrEngine3PeakThresholdScaler_xM >> 24) & 0xFF);

        armData[16] = (uint8_t)(cfrCtrlConfig[cfrCtrlCfgIndex].cfrCorrectionThresholdScaler_xM & 0xFF);
        armData[17] = (uint8_t)((cfrCtrlConfig[cfrCtrlCfgIndex].cfrCorrectionThresholdScaler_xM >> 8) & 0xFF);
        armData[18] = (uint8_t)((cfrCtrlConfig[cfrCtrlCfgIndex].cfrCorrectionThresholdScaler_xM >> 16) & 0xFF);
        armData[19] = (uint8_t)((cfrCtrlConfig[cfrCtrlCfgIndex].cfrCorrectionThresholdScaler_xM >> 24) & 0xFF);

        recoveryAction  = adrv9025_CfrCtrlCmdExecute(device,
                                                     cfrCtrlConfig[cfrCtrlCfgIndex].txChannelMask,
                                                     CFR_TARGET_THRESH_ID_NO_FLOAT,
                                                     &armData[0],
                                                     (uint8_t)20);
        ADI_ERROR_REPORT(&device->common,
                         ADI_COMMON_ERRSRC_API,
                                         ...
                                         adi_adrv9025_CfrCtrlConfig_v2_t* cfrCtrlConfig)
{
    ...
    cfrCtrlConfig->cfrPeakThreshold_xM = (uint32_t)((uint32_t)armData[0] |
                                                (uint32_t)armData[1] << 8 |
                                                (uint32_t)armData[2] << 16 |
                                                (uint32_t)armData[3] << 24);

    cfrCtrlConfig->cfrEngine1PeakThresholdScaler_xM = (uint32_t)((uint32_t)armData[4] |
                                                (uint32_t)armData[5] << 8 |
                                                (uint32_t)armData[6] << 16 |
                                                (uint32_t)armData[7] << 24);

    cfrCtrlConfig->cfrEngine2PeakThresholdScaler_xM = (uint32_t)((uint32_t)armData[8] |
                                                (uint32_t)armData[9] << 8 |
                                                (uint32_t)armData[10] << 16 |
                                                (uint32_t)armData[11] << 24);

    cfrCtrlConfig->cfrEngine3PeakThresholdScaler_xM = (uint32_t)((uint32_t)armData[12] |
                                                (uint32_t)armData[13] << 8 |
                                                (uint32_t)armData[14] << 16 |
                                                (uint32_t)armData[15] << 24);

    cfrCtrlConfig->cfrCorrectionThresholdScaler_xM = (uint32_t)((uint32_t)armData[16] |
                                                (uint32_t)armData[17] << 8 |
                                                (uint32_t)armData[18] << 16 |
                                                (uint32_t)armData[19] << 24);
                                      ...
                                      uint8_t                      clgcConfigArraySize)
{
...
#if ADI_ADRV9025_CLGC_RANGE_CHECK > 0
    adrv9025_ClgcConfigSetRangeCheck_v2(device,
                                        clgcConfig,
                                        clgcConfigArraySize);
    ADI_ERROR_RETURN(device->common.error.newAction);
#endif
    ...
    for (clgcCfgIndex = 0; clgcCfgIndex < clgcConfigArraySize; clgcCfgIndex++)
    {
        recoveryAction = adrv9025_ClgcConfigSet_v2(device,
                                                   &clgcConfig[clgcCfgIndex]);
        ADI_ERROR_REPORT(&device->common,
                         ADI_COMMON_ERRSRC_API,
                         device->common.error.errCode,
                                      ...
                                      adi_adrv9025_ClgcConfig_v2_t* clgcConfig)
{
    ...
    clgcConfig->clgcTxQualifyingThreshold_mdB = (uint32_t)((uint32_t)armConfigData[16] |
                                    ((uint32_t)armConfigData[17] << 8) |
                                    ((uint32_t)armConfigData[18] << 16) |
                                    ((uint32_t)armConfigData[19] << 24));

    clgcConfig->clgcOrxQualifyingThreshold_mdB = (uint32_t)((uint32_t)armConfigData[20] |
                                    ((uint32_t)armConfigData[21] << 8) |
                                    ((uint32_t)armConfigData[22] << 16) |
                                    ((uint32_t)armConfigData[23] << 24));

    clgcConfig->clgcExpectedLoopGainRipple_mdB = (uint32_t)((uint32_t)armConfigData[24] |
                                    ((uint32_t)armConfigData[25] << 8) |
                                    ((uint32_t)armConfigData[26] << 16) |
                                    ((uint32_t)armConfigData[27] << 24));

    clgcConfig->clgcExpectedLoopGain_mdB = (uint32_t)((uint32_t)armConfigData[28] |
                                    ((uint32_t)armConfigData[29] << 8) |
                                    ((uint32_t)armConfigData[30] << 16) |
                                    ((uint32_t)armConfigData[31] << 24));

    clgcConfig->clgcMaxGainAdjustmentStepSize_mdB = (uint32_t)((uint32_t)armConfigData[32] |
                                    ((uint32_t)armConfigData[33] << 8) |
                                    ((uint32_t)armConfigData[34] << 16) |
                                    ((uint32_t)armConfigData[35] << 24));

    clgcConfig->clgcMinTxAttenAdjust_mdB = (uint32_t)((uint32_t)armConfigData[36] |
                                   ((uint32_t)armConfigData[37] << 8) |
                                   ((uint32_t)armConfigData[38] << 16) |
                                   ((uint32_t)armConfigData[39] << 24));

    clgcConfig->clgcMaxTxAttenAdjust_mdB = (uint32_t)((uint32_t)armConfigData[40] |
                                   ((uint32_t)armConfigData[41] << 8) |
                                   ((uint32_t)armConfigData[42] << 16) |
                                   ((uint32_t)armConfigData[43] << 24));

    clgcConfig->clgcDampingParam_xM   = (uint32_t)((uint32_t)armConfigData[44] |
                                    ((uint32_t)armConfigData[45] << 8) |
                                    ((uint32_t)armConfigData[46] << 16) |
                                    ((uint32_t)armConfigData[47] << 24));
                                      ...
                                      adi_adrv9025_ClgcStatus_v2_t* clgcStatus)
{
     ...
     int32_t recoveryAction = ADI_COMMON_ACT_NO_ACTION;
     uint8_t extData[3] = { ADRV9025_CPU_OBJECTID_CAL_STATUS, ADRV9025_CPU_OBJECTID_CLGC_TRACKING_NO_FLOAT, 0 };
     uint8_t armData[140] = { 0 };
     uint8_t cmdStatusByte = 0;
    ...
    adi_adrv9025_CpuMemRead(device,
     ...
     clgcStatus->clgcTxRmsPower_mdB = (uint32_t)((uint32_t)armData[88] |
                                     ((uint32_t)armData[89] << 8) |
                                     ((uint32_t)armData[90] << 16) |
                                     ((uint32_t)armData[91] << 24));

     clgcStatus->clgcOrxRmsPower_mdB = (uint32_t)((uint32_t)armData[92] |
                                     ((uint32_t)armData[93] << 8) |
                                     ((uint32_t)armData[94] << 16) |
                                     ((uint32_t)armData[95] << 24));

     clgcStatus->clgcLoopGain_mdB = (uint32_t)((uint32_t)armData[96] |
                                     ((uint32_t)armData[97] << 8) |
                                     ((uint32_t)armData[98] << 16) |
                                     ((uint32_t)armData[99] << 24));
                                                                ...
                                                                uint32_t               dataCount)
{
    ...
    static const uint32_t PM_ARMD_CUSTOM_WAVEFORM_BUF = ADRV9025_CPI_D_ADDR_CUSTOM_WAVEFORM_BUF;

    int32_t recoveryAction = ADI_COMMON_ACT_NO_ACTION;
    uint8_t armData[8192] = { 0 }; /* 8192 should be the max  */
