...
static void adrv903x_shutdown(struct adrv903x_rf_phy *phy)
{
	...
	adi_adrv903x_Shutdown(phy->kororDevice);
...
static void adrv903x_info(struct adrv903x_rf_phy *phy)
{
	...
	adi_adrv903x_ApiVersionGet(phy->kororDevice, &apiVersion);
	adi_adrv903x_DeviceRevGet(phy->kororDevice, &siRevision);

	pr_info("\nadrv903x-phy Rev %d, API version: %u.%u.%u.%u\n\n",
					...
					enum jesd204_state_op_reason reason)
{
	...
	recoveryAction = adi_adrv903x_HwOpen(phy->kororDevice, &phy->spiSettings);
	...
	recoveryAction = adi_adrv903x_HwReset(phy->kororDevice);

	recoveryAction = adi_adrv903x_PreMcsInit(phy->kororDevice, &deviceInitStruct,
	...
	recoveryAction = adi_adrv903x_PreMcsInit_NonBroadcast(phy->kororDevice,
			 &deviceInitStruct);
	if (recoveryAction != ADI_ADRV903X_ERR_ACT_NONE) {
				      ...
				      struct jesd204_link *lnk)
{
	...
	case DEFRAMER1_LINK_TX:
		...
		recoveryAction = adi_adrv903x_DeframerCfgGet(phy->kororDevice, source_id,
				 &deframerCfg);
		if (recoveryAction != ADI_ADRV903X_ERR_ACT_NONE) {
	...
	case FRAMER2_LINK_RX:
		...
		recoveryAction = adi_adrv903x_FramerCfgGet(phy->kororDevice, source_id,
				 &framerCfg);
		if (recoveryAction != ADI_ADRV903X_ERR_ACT_NONE) {
				...
				enum jesd204_state_op_reason reason)
{
	...
	if (reason == JESD204_STATE_OP_REASON_UNINIT) {
		...
		adi_adrv903x_HwClose(phy->kororDevice);
	...
	recoveryAction = adi_adrv903x_MultichipSyncSet(phy->kororDevice, ADI_ENABLE);
	if (recoveryAction != ADI_ADRV903X_ERR_ACT_NONE) {
		pr_err("ERROR adi_adrv903x_MultichipSyncSet failed in %s at line %d.\n",
		       __func__, __LINE__);
		return JESD204_STATE_CHANGE_ERROR;
	}
...
static int adrv903x_jesd204_setup_stage1(struct jesd204_dev *jdev,
		enum jesd204_state_op_reason reason)
{
	adi_adrv903x_ErrAction_e recoveryAction = ADI_ADRV903X_ERR_ACT_RESET_DEVICE;
	struct adrv903x_jesd204_priv *priv = jesd204_dev_priv(jdev);
	struct adrv903x_rf_phy *phy = priv->phy;
	uint32_t mcsStatus;
	int i;
	...
	for (i = 0; i < 255; i++) {
		recoveryAction = adi_adrv903x_MultichipSyncStatusGet(phy->kororDevice,
				 &mcsStatus);
		if (recoveryAction) {
		...
		enum jesd204_state_op_reason reason)
{
	...
	/* MCS end sequence*/
	recoveryAction = adi_adrv903x_MultichipSyncSet(phy->kororDevice, ADI_DISABLE);
	if (recoveryAction) {
		pr_err("ERROR adi_adrv903x_MultichipSyncSet failed in %s at line %d.\n",
		       __func__, __LINE__);
		return JESD204_STATE_CHANGE_ERROR;
	...
	/* Post MCS */
	recoveryAction = adi_adrv903x_PostMcsInit(phy->kororDevice,
			 &utilityInit);
	if (recoveryAction) {
		pr_err("ERROR adi_adrv903x_PostMcsInit failed in %s at line %d.\n", __func__,
		       __LINE__);
		return JESD204_STATE_CHANGE_ERROR;
	}
					...
					struct jesd204_link *lnk)
{
	...
	if (priv->link[lnk->link_id].is_framer) {
		if (phy->kororDevice->devStateInfo.linkSharingEnabled == 1) {
			ret = adi_adrv903x_FramerSysrefCtrlSet(phy->kororDevice,
							       ADI_ADRV903X_FRAMER_1, ADI_DISABLE);
			if (ret) {
				pr_err("ERROR adi_adrv903x_FramerSysrefCtrlSet failed in %s at line %d.\n",
				       __func__, __LINE__);
				return JESD204_STATE_CHANGE_ERROR;
			}

			ret = adi_adrv903x_FramerLinkStateSet(phy->kororDevice,
							      ADI_ADRV903X_FRAMER_1, ADI_DISABLE);
			if (ret) {
				pr_err("ERROR adi_adrv903x_FramerLinkStateSet failed in %s at line %d.\n",
				       __func__, __LINE__);
				return JESD204_STATE_CHANGE_ERROR;
			}
		...
		ret = adi_adrv903x_SerializerReset(phy->kororDevice);
		if (ret) {
			pr_err("adi_adrv903x_SerializerReset failedin %s at line %d.\n",
			       __func__, __LINE__);
			return JESD204_STATE_CHANGE_ERROR;
		}

	} else {
		ret = adi_adrv903x_DeframerSysrefCtrlSet(phy->kororDevice,
				(uint8_t) ADI_ADRV903X_ALL_DEFRAMER, ADI_DISABLE);
		if (ret) {
			pr_err("ERROR adi_adrv903x_DeframerSysrefCtrlSet failed in %s at line %d.\n",
			       __func__, __LINE__);
			return JESD204_STATE_CHANGE_ERROR;
		}

		ret = adi_adrv903x_DeframerLinkStateSet(phy->kororDevice,
							(uint8_t) ADI_ADRV903X_ALL_DEFRAMER, ADI_DISABLE);
		if (ret) {
			pr_err("ERROR adi_adrv903x_DeframerLinkStateSet failed in %s at line %d.\n",
			       __func__, __LINE__);
			return JESD204_STATE_CHANGE_ERROR;
		}
	};
					...
					struct jesd204_link *lnk)
{
	...
	if (!priv->link[lnk->link_id].is_framer) { /* DEFRAMER */
		...
		for (int j = 0U; j < ADI_ADRV903X_MAX_DEFRAMERS; ++j) {
			ret = adi_adrv903x_DeframerCfgGet(phy->kororDevice,
							  (adi_adrv903x_DeframerSel_e)(1U << j), &deframerCfg);

			desLaneMask |= deframerCfg.deserializerLanesEnabled;
		}

		/* Notify ARM to run SERDES Calbriation if necessary */
		ret = adi_adrv903x_InitCalsRun(phy->kororDevice, &serdesCal);
		if (ret) {
			pr_err("ERROR adi_adrv903x_InitCalsRun failed in %s at line %d.\n",
			       __func__, __LINE__);
			return JESD204_STATE_CHANGE_ERROR;
		}

		/* Wait up to 60 seconds for ARM */
		ret = adi_adrv903x_InitCalsWait(phy->kororDevice, 60000);
		if (ret) {
			pr_err("ERROR adi_adrv903x_InitCalsWait failed in %s at line %d.\n", __func__,
			       __LINE__);
			return JESD204_STATE_CHANGE_ERROR;
		}
		...
		struct jesd204_link *lnk)
{
	...
	if (priv->link[lnk->link_id].is_framer) {
		ret = adi_adrv903x_FramerStatusGet(phy->kororDevice,
						   priv->link[lnk->link_id].source_id, &framerStatus);
		if (ret) {
			pr_err("ERROR adi_adrv903x_FramerStatusGet failed in %s at line %d.\n",
			       __func__, __LINE__);
			return JESD204_STATE_CHANGE_ERROR;
		}
		...
		ret = adi_adrv903x_DeframerStatusGet_v2(phy->kororDevice,
							priv->link[lnk->link_id].source_id, &deframerStatus);
		if (ret) {
			pr_err("ERROR adi_adrv903x_DeframerStatusGet failed in %s at line %d.\n",
			       __func__, __LINE__);
			return JESD204_STATE_CHANGE_ERROR;
		}
 ...
 * Returns init cal.
 */
ADI_API adi_adrv903x_InitCalibrations_e example_cals_TrackingCalConvert(
	const adi_adrv903x_TrackingCalibrationMask_e trackingCal)
{
		...
		enum jesd204_state_op_reason reason)
{
	adi_adrv903x_TrackingCalibrationMask_e trackingCal =
		(adi_adrv903x_TrackingCalibrationMask_e) 0U;
	adi_adrv903x_InitCalibrations_e currentInitCalMask =
		(adi_adrv903x_InitCalibrations_e) 0U;
	struct adrv903x_jesd204_priv *priv = jesd204_dev_priv(jdev);
	...
	ret = adi_adrv903x_RxTxEnableSet(phy->kororDevice, 0x00, 0x00,
					 ADI_ADRV903X_RX_MASK_ALL, ADI_ADRV903X_RX_MASK_ALL,
					 ADI_ADRV903X_TXALL, ADI_ADRV903X_TXALL);
	if (ret) {
		pr_err("ERROR adi_adrv903x_RxTxEnableSet failed in %s at line %d.\n",
		       __func__, __LINE__);
		return JESD204_STATE_CHANGE_ERROR;
	}

	for (uint8_t chan = 0; chan < 8; chan++) {
		txAttenuation[0].txChannelMask = ADI_ADRV903X_TX0 << chan;
		txAttenuation[0].txAttenuation_mdB = 6000;
		ret = adi_adrv903x_TxAttenSet(phy->kororDevice, txAttenuation, 1);
		if (ret != ADI_ADRV903X_ERR_ACT_NONE) {
			pr_err("ERROR adi_adrv903x_TxAttenSet failed in %s at line %d.\n", __func__,
			       __LINE__);
			return -1;
		}
	}
	...
	ret = adi_adrv903x_InitCalsDetailedStatusGet(phy->kororDevice, &initCalStatus);
	if (ret) {
		pr_err("ERROR adi_adrv903x_InitCalsDetailedStatusGet failed in %s at line %d.\n",
		       __func__, __LINE__);
		return JESD204_STATE_CHANGE_ERROR;
	}
	...
	for (i = 0U; i < NUM_TRACKING_CALS; ++i) {
		...
		ret = adi_adrv903x_TrackingCalsEnableSet(phy->kororDevice,
				trackingCal,
				ALL_CHANNELS_MASK,
 ...
 */
static int populateTrxFileInfo(adi_adrv903x_TrxFileInfo_t * const
			       trxBinaryInfoPtr,
...

{
	...
	trxBinaryInfoPtr->stream.filePath[ADI_ADRV903X_MAX_FILE_LENGTH - 1] = '\0';
	strncpy((char *)trxBinaryInfoPtr->cpu.filePath, cpuBin,
		ADI_ADRV903X_MAX_FILE_LENGTH);
	trxBinaryInfoPtr->cpu.filePath[ADI_ADRV903X_MAX_FILE_LENGTH - 1] = '\0';
	strncpy((char *)trxBinaryInfoPtr->cpuProfile.filePath, configBin,
		ADI_ADRV903X_MAX_FILE_LENGTH);
	trxBinaryInfoPtr->cpuProfile.filePath[ADI_ADRV903X_MAX_FILE_LENGTH - 1] = '\0';

	for (unsigned i = 0; i < ADI_ADRV903X_RX_GAIN_TABLE_ARR_MAX; i++) {
		trxBinaryInfoPtr->rxGainTable[i].channelMask = 1 << i;
		strncpy((char *)trxBinaryInfoPtr->rxGainTable[i].filePath, gainTable,
			ADI_ADRV903X_MAX_FILE_LENGTH);
		trxBinaryInfoPtr->rxGainTable[i].filePath[ADI_ADRV903X_MAX_FILE_LENGTH - 1] =
			'\0';
	}
...
*/
int32_t adrv903x_init(struct adrv903x_rf_phy **dev,
		      const struct adrv903x_init_param *init_param)
{
	const char configBin[ADI_ADRV903X_MAX_FILE_LENGTH] =
		"ADRV9030_DeviceProfileTest.bin";
	const char streamBin[ADI_ADRV903X_MAX_FILE_LENGTH] =
		"ADRV9030_stream_image.bin";
	const char gainTable[ADI_ADRV903X_MAX_FILE_LENGTH] = "ADRV9030_RxGainTable.csv";
	const char cpuBin[ADI_ADRV903X_MAX_FILE_LENGTH] = "ADRV9030_FW.bin";
	const char radSeqBin[ADI_ADRV903X_MAX_FILE_LENGTH] = "";
	adi_adrv903x_SpiConfigSettings_t *spi_settings;
	adi_adrv903x_SpiOptions_t *spi_options;
	struct adrv903x_jesd204_priv *priv;
	struct adrv903x_rf_phy *phy;
	int ret;

	phy = (struct adrv903x_rf_phy *)no_os_calloc(1, sizeof(*phy));
	if (!phy)
		goto error;

	phy->kororDevice = init_param->adrv903x_device;
	phy->spi_device_id = 0;
	phy->device_id = 0;
	phy->dev_clk = init_param->dev_clk;

	spi_settings = adrv903x_spi_settings_get();
	spi_options = adrv903x_spi_options_get();
	...
	ret = populateTrxFileInfo(&phy->trxBinaryInfoPtr, &cpuBin[0], &configBin[0],
				  &streamBin[0], &gainTable[0], &radSeqBin[0]);
	if (ret) {
	...
	ret = adrv903x_setup(phy);
	if (ret < 0) {
		pr_err("%s: adrv903x_setup failed (%d)\n", __func__, ret);
		goto error_setup;
	}

	ret = jesd204_dev_register(&phy->jdev, &jesd204_adrv903x_init);
	if (ret)
		goto error_setup;
...
static int32_t adrv9030_bb_recalc_rate(struct no_os_clk_desc *desc,
				       uint64_t *rate)
{
	struct adrv903x_rf_phy *adrv903x_dev;
	adrv903x_dev = desc->dev_desc;

	if (!strcmp(desc->name, "-rx_sampl_clk"))
		*rate = adrv903x_dev->rx_iqRate_kHz;
	else if (!strcmp(desc->name, "-tx_sampl_clk"))
		*rate = adrv903x_dev->tx_iqRate_kHz;
	else if (!strcmp(desc->name, "-orx_sampl_clk"))
		*rate = adrv903x_dev->orx_iqRate_kHz;
	else
		return -EINVAL;
...
static int32_t adrv9030_bb_set_rate(struct no_os_clk_desc *desc,
				    uint64_t rate)
{
	// Do nothing
	return 0;
...
static int32_t adrv9030_bb_round_rate(struct no_os_clk_desc *desc,
				      uint64_t rate,
...
static const struct no_os_clk_platform_ops adrv9030_bb_clk_ops = {
	.clk_round_rate = &adrv9030_bb_round_rate,
	.clk_set_rate = &adrv9030_bb_set_rate,
	.clk_recalc_rate = &adrv9030_bb_recalc_rate,
};
...
int adrv903x_setup(struct adrv903x_rf_phy *phy)
{
	...
	const char *names[NUM_ADRV903X_CLKS] = {
		"-rx_sampl_clk", "-tx_sampl_clk", "-orx_sampl_clk"
	};
	int recoveryAction = ADI_ADRV903X_ERR_ACT_NONE;
	...
	const char *dev_name = "ADRV9030";
	adi_adrv903x_Version_t apiVersion;
	adi_common_ErrData_t* errPtr;
	uint8_t siRevision = 0xbb;
	int ret;

	phy->kororDevice = &phy->adi_adrv903x_device;
	phy->kororDevice->common.devHalInfo = &phy->hal;

	ADI_APP_MESSAGE("adrv903x_setup()\n");
	...
	ret = adi_adrv903x_HwOpen(phy->kororDevice, adrv903x_spi_settings_get());
	if (ret)
		return ret;
	...
	clk_init.name = names[ADRV903X_RX_SAMPL_CLK];
	clk_init.hw_ch_num = 1;
	clk_init.platform_ops = &adrv9030_bb_clk_ops;
	...
	phy->clks[ADRV903X_RX_SAMPL_CLK] = rx_sample_clk;
	...
	/* Initialize clk component */
	clk_init.name = names[ADRV903X_TX_SAMPL_CLK];
	clk_init.hw_ch_num = 1;
	clk_init.platform_ops = &adrv9030_bb_clk_ops;
	clk_init.dev_desc = phy;
	...
	phy->clks[ADRV903X_TX_SAMPL_CLK] = tx_sample_clk;
	...
	/* Initialize clk component */
	clk_init.name = names[ADRV903X_ORX_SAMPL_CLK];
	clk_init.hw_ch_num = 1;
	clk_init.platform_ops = &adrv9030_bb_clk_ops;
	clk_init.dev_desc = phy;
	...
	phy->clks[ADRV903X_ORX_SAMPL_CLK] = orx_sample_clk;

	ret = adi_adrv903x_SpiVerify(phy->kororDevice);
	if (ret)
		goto orx_out_clk_init_error;

	ret = adi_adrv903x_ApiVersionGet(phy->kororDevice, &apiVersion);
	if (ret)
		goto orx_out_clk_init_error;

	ret = adi_adrv903x_DeviceRevGet(phy->kororDevice, &siRevision);
	if (ret)
		goto orx_out_clk_init_error;
	...
	pr_info("adrv903x-device revision: %#x\n", siRevision);

	if (apiVersion.majorVer > 1U) {
		ADI_APP_API_CALL_RETURN(recoveryAction, adi_adrv903x_InitDataExtract,
					phy->kororDevice,
	...
	recoveryAction = adi_adrv903x_DeviceInfoExtract(phy->kororDevice,
			 &phy->trxBinaryInfoPtr.cpuProfile);
	if (recoveryAction != ADI_ADRV903X_ERR_ACT_NONE) {
		ADI_API_ERROR_REPORT(&phy->kororDevice->common, recoveryAction,
