/**
 * \file
 * \brief Contains ADRV910X DPD data types
 *
...
#ifndef _ADI_ADRV910X_DPD_TYPES_H_
#define _ADI_ADRV910X_DPD_TYPES_H_
...
#define ADI_ADRV910X_DPD_NUM_COEFFICIENTS 208
 ...
 */
typedef enum adi_adrv910x_DpdAmplifier
{
    ADI_ADRV910X_DPDAMPLIFIER_NONE,
    ADI_ADRV910X_DPDAMPLIFIER_DEFAULT,
    ADI_ADRV910X_DPDAMPLIFIER_GAN
} adi_adrv910x_DpdAmplifier_e;

/**
 * \brief Supported DPD LUT sizes
 */
typedef enum adi_adrv910x_DpdLutSize
{
    ADI_ADRV910X_DPDLUTSIZE_256,
    ADI_ADRV910X_DPDLUTSIZE_512
} adi_adrv910x_DpdLutSize_e;

/**
 * \brief Supported DPD models
 */
typedef enum adi_adrv910x_DpdModel
{
    ADI_ADRV910X_DPDMODEL_0 = 0,
    ADI_ADRV910X_DPDMODEL_1 = 1,
    ADI_ADRV910X_DPDMODEL_3 = 3,
    ADI_ADRV910X_DPDMODEL_4 = 4
} adi_adrv910x_DpdModel_e;

/**
 * \brief DPD settings that must be configured in the STANDBY state
 */
typedef struct adi_adrv910x_DpdInitCfg
{
    bool enable;                                    /*!< When true, the DPD actuator is placed in the datapath and
                                                     *   ADI_ADRV910X_TRACKING_CAL_TX_DPD_CLGC may be used to enable DPD */
    adi_adrv910x_DpdAmplifier_e amplifierType;      //!< Type of Power Amplifier
    adi_adrv910x_DpdLutSize_e lutSize;              //!< Bit size of the DPD LUT
    adi_adrv910x_DpdModel_e model;                  //!< DPD Model to be used
    bool changeModelTapOrders;                      //!< Whether to use the Tap_Orders in #modelOrdersForEachTap
    int32_t modelOrdersForEachTap[4];              /*!< Bitmap for each of the taps in a model to indicate which power
...
typedef struct adi_adrv910x_DpdCfg
{
    ...
    uint32_t dpdSamplingRate_Hz;            /*!< sampling rate in Hz for the DPD actuator and capture.
                                                 'dpdSamplingRate_Hz' is read only and is ignored in adi_adrv910x_dpd_Configure() */
...
} adi_adrv910x_DpdCfg_t;

typedef struct adi_adrv910x_DpdCoefficients
{
    uint8_t region;     //!< The region of the LUT initialization data (valid 0 - 7)
    uint8_t coefficients[ADI_ADRV910X_DPD_NUM_COEFFICIENTS];    //!< DPD coefficients
}adi_adrv910x_DpdCoefficients_t;

typedef struct adi_adrv910x_DpdFhRegions
{
...
} adi_adrv910x_DpdFhRegions_t;

typedef struct adi_adrv910x_DpdChannelStatus
{
    uint32_t numberOfSuccessfulIterations;  /*!< a count of successful estimations for DPD */
} adi_adrv910x_DpdChannelStatus_t;

#endif /* _ADI_ADRV910X_DPD_TYPES_H_ */
