...
static uint32_t calc_tx_misc_base(int32_t side);
static uint32_t calc_tx_lb_base(int32_t adc);

static int32_t adi_apollo_lb0_tx_xbar_map(adi_apollo_device_t *device, adi_apollo_adc_idx_e adc, adi_apollo_dac_idx_e dac, uint8_t *xbar);
...
int32_t adi_apollo_lb0_tx_xbar_set(adi_apollo_device_t *device, uint16_t sides, uint16_t adcs[], uint32_t adc_map_length)
{
    ...
    for (i = 0; i < adc_map_length; i++) {
        err = adi_apollo_lb0_tx_xbar_map(device, adcs[i], dacs[i], &xbar_value);
        ADI_APOLLO_ERROR_RETURN(err);
    }
...
static uint32_t calc_tx_misc_base(int32_t side) {
    static const uint32_t tx_misc_reg[ADI_APOLLO_NUM_SIDES] = {TX_MISC_TX_TOP_TX_DIGITAL0, TX_MISC_TX_TOP_TX_DIGITAL1};
    return tx_misc_reg[side];
}

static uint32_t calc_tx_lb_base(int32_t adc){
    static const uint32_t tx_lb_reg[ADI_APOLLO_ADC_NUM] = {TX_LOOPBACK0_TX_SLICE_0_TX_DIGITAL0, TX_LOOPBACK1_TX_SLICE_0_TX_DIGITAL0, TX_LOOPBACK0_TX_SLICE_1_TX_DIGITAL0, TX_LOOPBACK1_TX_SLICE_1_TX_DIGITAL0, 
                                                  TX_LOOPBACK0_TX_SLICE_0_TX_DIGITAL1, TX_LOOPBACK1_TX_SLICE_0_TX_DIGITAL1, TX_LOOPBACK0_TX_SLICE_1_TX_DIGITAL1, TX_LOOPBACK1_TX_SLICE_1_TX_DIGITAL1};
