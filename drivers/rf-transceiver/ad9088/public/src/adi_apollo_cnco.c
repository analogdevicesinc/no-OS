            ...
            for(j = first; j < first+length; j++) {
                 ...
                 */
                {
                    ...
                    if (word_sel == ADI_APOLLO_NCO_PROFILE_PHASE_INCREMENT)
                        err = adi_apollo_hal_paged_bf_set(device, BF_DRC_PHASE_INC_INFO(regmap_base_addr), w);
                    else
                        err = adi_apollo_hal_paged_bf_set(device, BF_DRC_PHASE_OFFSET_INFO(regmap_base_addr), w);
                    ADI_APOLLO_ERROR_RETURN(err);
                }
#else
                if (word_sel == ADI_APOLLO_NCO_PROFILE_PHASE_INCREMENT) {
