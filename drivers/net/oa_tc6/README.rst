...
is ``CONFIG_OA_RX_FRAME_BUFF_NUM`` (default 10).
...
.. list-table::
   ...
   * - ``user_tx_frame_buffer[N]``
     - N × ~1526 B
     - TX frame pool. N = ``CONFIG_OA_TX_FRAME_BUFF_NUM`` (default 10).
       Each entry holds a full Ethernet frame (``CONFIG_OA_CHUNK_BUFFER_SIZE``
       = 1514 B data) plus metadata.
   * - ``user_rx_frame_buffer[N]``
     - N × ~1526 B
     - RX frame pool. N = ``CONFIG_OA_RX_FRAME_BUFF_NUM`` (default 10).
   * - ``ctrl_lock``, ``data_lock``
     - platform-dependent
     - Mutexes for serialising control and data path access.

With default settings the descriptor is approximately **32 KB**. The
dominant cost is the 20 frame buffers (10 TX + 10 RX) at ~1.5 KB each.
...
via compiler flags) to override defaults:
