...
* ``OA_TC6_EVENT_RX`` — a frame was received.
...
The buffer pools and service limits are tunable by overriding the macros
before including the header. A subset is also exposed through Kconfig.

Exposed through Kconfig:

* ``CONFIG_OA_TX_FRAME_BUFF_NUM`` (default 2) — transmit buffer pool depth.
* ``CONFIG_OA_RX_FRAME_BUFF_NUM`` (default 5) — receive buffer pool depth.
...
Header macros only (no Kconfig option):
