...
chunked SPI transactions. Frames shorter than the 60-byte minimum
payload are padded on TX; the MAC then appends the 4-byte FCS, so the
minimum 64-byte Ethernet frame is transmitted on the wire.
...
``adin1140_poll()`` drains pending TX/RX work and is typically called
from the main loop, either periodically or in response to the MAC-PHY's
INT pin (GPIO IRQ). ``adin1140_link_state()`` reads ``STATUS1`` to report
the current link state.

Statistics
~~~~~~~~~~

``adin1140_get_stats()`` reads the device's on-chip counters into
``struct adin1140_stats``: ``rx_frames`` (RX frame count),
``tx_frames`` (TX frame count), ``rx_crc_err`` (frames dropped for a bad
FCS), and ``rx_align_err`` (frames received with an alignment error).
These are hardware counters read directly from the MAC-PHY, distinct from
the software counters kept by the OA-TC6 layer.
