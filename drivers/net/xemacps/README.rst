...
configuration, buffer descriptors), brings up the PHY via MDIO
auto-negotiation, and starts the MAC. The ``xemacps_remove()`` function
stops the MAC and frees the descriptor.
...
Polling and Errata Handling
~~~~~~~~~~~~~~~~~~~~~~~~~~~

The ``xemacps_poll()`` function must be called once per polling cycle
before the ``xemacps_recv()`` loop. It reads the RX status register,
runs the AR# 52028 errata workaround (GEM v2 only), and clears any
error flags.
