...
The ``adiol100_init_param`` struct contains the settings needed for
**adiol100_init**:
...
.. list-table::
   ...
   * - ``spi_ip``
     - pointer
     - SPI bus configuration (device ID, speed, mode 0, chip select,
       platform ops). Required.
   * - ``chip_addr``
     - uint8_t
     - Device address set by the ADRSEL pin (0-3). On the ADIOL100 EVKIT
       this corresponds to the J10 jumper setting. The driver rejects
       values > 3 with ``-EINVAL``.
   * - ``clock_src``
     - uint8_t
     - Clock source selection: ``ADIOL100_CLK_INTERNAL`` (default on
       power-up), ``ADIOL100_CLK_CRYSTAL``, or ``ADIOL100_CLK_EXTERNAL``.
   * - ``clk_div``
     - uint8_t
     - Clock divider applied to the clock output (ClkDiv[1:0], 0--3).
   * - ``gpio_irqg``
     - pointer
     - Optional GPIO init params for the global interrupt pin (IRQG).
       Set to NULL if not used.
   * - ``gpio_irqa``
     - pointer
     - Optional GPIO init params for the channel A interrupt pin (IRQA).
       Set to NULL if not used.
   * - ``gpio_irqb``
     - pointer
     - Optional GPIO init params for the channel B interrupt pin (IRQB).
       Set to NULL if not used.

Use **adiol100_read_revid** and **adiol100_read_devid** to read the chip
RevisionID and DeviceID registers. These are called automatically during
init but can also be used independently.

Use **adiol100_setup_clock** to configure the clock source (crystal,
external, or internal), divider, and enable clock output. This is also
called during init with the values from the init parameters.
...
Use **adiol100_config_framer** to configure the IO-Link framer. It writes both
FramCtrl1 (checksum insertion, framer enable) and FramCtrl2 (CKT error inhibit,
message repeat, byte delay, device answer delay, TX size error inhibit).
...
160 ms. Returns 0 on success, ``-ENODEV`` if no device responded,
...
Use **adiol100_set_burst_len** to configure the SPI burst length for FIFO
access on a given channel. This is called internally by **adiol100_load_msg**
but can also be used directly for advanced FIFO control.

Use **adiol100_get_fifo_status** to read the RxFIFO status register for a
channel, which includes the FIFO level and error flags.

Use **adiol100_get_status** to read the channel status register.
...
Use **adiol100_get_cycle_tmr** to read the current cycle time register value.
...
The driver optionally configures the IRQG, IRQA, and IRQB GPIO pins as
inputs during **adiol100_init**. To enable this, pass ``gpio_irqg``,
``gpio_irqa``, and/or ``gpio_irqb`` in the init parameters. All GPIO fields
default to NULL, in which case no GPIO setup is performed. Each GPIO is
independent, so you can pass only the ones you need.

The driver does not manage the IRQ controller, callback registration, or
interrupt enable/disable. These are the responsibility of the application or
project code. This separation keeps the driver platform-agnostic: the
application sets up the IRQ controller (and any platform-specific NVIC
configuration), registers its own ISR using the GPIO descriptors from the
driver's device structure (``dev->gpio_irqa``, etc.), and enables the
interrupts.

The driver's register access functions (``adiol100_get_channel_irq``,
``adiol100_enable_channel_irq``, etc.) work regardless of how the GPIO
interrupt is managed.

.. code-block:: c

	/* After adiol100_init() with gpio_irqa set: */
	no_os_irq_ctrl_init(&irq_ctrl, &gpio_irq_ip);
	...
	no_os_irq_register_callback(irq_ctrl, dev->gpio_irqa->number, &cb);
	no_os_irq_trigger_level_set(irq_ctrl, dev->gpio_irqa->number,
				    NO_OS_IRQ_EDGE_FALLING);
	no_os_irq_enable(irq_ctrl, dev->gpio_irqa->number);
...
on when CQ/DI is high, static on when CQ/DI is low, or fast blink.

Use **adiol100_reset_leds** to reset all LED registers to their default
values (all LEDs off).
...
Use **adiol100_global_reset** to reset all LEDs to off, disable all global
interrupts, clear the trigger register, and read-clear all interrupt and
status registers for both channels. This does not reset register values to
defaults.
...
.. code-block:: c
	...
	int main(void)
	{
		...
		ret = adiol100_config_framer(dev, ADIOL100_CH_A,
					     ADIOL100_CHKS_EN,
					     ADIOL100_FRAMER_EN,
					     ADIOL100_CKTERR_SEND,
					     ADIOL100_MSGREP_DIS,
					     ADIOL100_BDELAY_3TBIT,
					     0x08,
					     ADIOL100_TSIZEERR_SEND);
		if (ret)
			goto cleanup;
