			 ...
			 uint8_t clock_src, uint8_t clk_div)
{
	...
	mask = ADIOL100_CLKOEN | ADIOL100_CLKDIV_MSK | ADIOL100_CLOCKCFG_MSK;
	val = ADIOL100_CLKOEN |
	      no_os_field_prep(ADIOL100_CLKDIV_MSK, clk_div) |
	      no_os_field_prep(ADIOL100_CLOCKCFG_MSK, clock_src);

	return adiol100_update(dev, ADIOL100_REG_CLOCKCFG, mask, val);
}

/**
 * @brief Set up a GPIO as input for IRQ use.
 * @param gpio_desc - Pointer to store the GPIO descriptor.
 * @param gpio_ip   - GPIO init parameters.
 * @return 0 in case of success, negative error code otherwise.
 */
static int adiol100_setup_gpio_input(struct no_os_gpio_desc **gpio_desc,
...
int adiol100_init(struct adiol100_dev **dev, struct adiol100_init_param *ip)
{
	...
	if (ip->chip_addr > ADIOL100_CHIP_ADDR_MAX) {
		ret = -EINVAL;
		goto err;
	}
	...
	ret = adiol100_config_cq_protection(descriptor, ADIOL100_CH_A,
					    ADIOL100_CQCL_300MA,
					    ADIOL100_CLBL_1MS,
					    ADIOL100_ARTTMO_5MS,
	...
	ret = adiol100_config_cq_protection(descriptor, ADIOL100_CH_B,
					    ADIOL100_CQCL_300MA,
					    ADIOL100_CLBL_1MS,
					    ADIOL100_ARTTMO_5MS,
					    ADIOL100_AUTORETRY_DIS,
	...
	if (ip->gpio_irqg) {
		ret = adiol100_setup_gpio_input(&descriptor->gpio_irqg,
						ip->gpio_irqg);
		if (ret)
			goto err;
	}
	...
	if (ip->gpio_irqa) {
		ret = adiol100_setup_gpio_input(&descriptor->gpio_irqa,
						ip->gpio_irqa);
		if (ret)
			goto err_gpio_a;
	}
	...
	if (ip->gpio_irqb) {
		ret = adiol100_setup_gpio_input(&descriptor->gpio_irqb,
						ip->gpio_irqb);
		if (ret)
			goto err_gpio_b;
	}
...
err_gpio_b:
	if (descriptor->gpio_irqa)
		no_os_gpio_remove(descriptor->gpio_irqa);
err_gpio_a:
	if (descriptor->gpio_irqg)
		no_os_gpio_remove(descriptor->gpio_irqg);
err:
...
int adiol100_remove(struct adiol100_dev *dev)
{
	...
	if (dev->gpio_irqb)
		no_os_gpio_remove(dev->gpio_irqb);

	if (dev->gpio_irqa)
		no_os_gpio_remove(dev->gpio_irqa);

	if (dev->gpio_irqg)
		no_os_gpio_remove(dev->gpio_irqg);
...
int adiol100_read(struct adiol100_dev *dev, uint16_t reg, uint16_t *value)
{
	...
	paged = (reg & ADIOL100_REG_PAGE_MSK) == ADIOL100_PAGED_BASE;

	if (paged) {
		buf[0] = ADIOL100_SPI_CMD(dev->chip_addr, ADIOL100_REG_EXTRAPAGE,
					  ADIOL100_READ);
		buf[1] = ADIOL100_REG_PAGE_OFFSET(reg);
		buf[2] = 0x00;
		buf[3] = 0x00;
		len = ADIOL100_SPI_PAGED_LEN;
	} else {
		buf[0] = ADIOL100_SPI_CMD(dev->chip_addr, reg, ADIOL100_READ);
		buf[1] = 0x00;
		buf[2] = 0x00;
		len = ADIOL100_SPI_DIRECT_LEN;
	}
	...
	*value = no_os_get_unaligned_be16(&buf[len - 2]);
	return 0;
}
...
int adiol100_write(struct adiol100_dev *dev, uint16_t reg, uint16_t value)
{
	...
	if (paged) {
		buf[0] = ADIOL100_SPI_CMD(dev->chip_addr, ADIOL100_REG_EXTRAPAGE,
					  ADIOL100_WRITE);
		buf[1] = ADIOL100_REG_PAGE_OFFSET(reg);
		no_os_put_unaligned_be16(value, &buf[2]);
		len = ADIOL100_SPI_PAGED_LEN;
	} else {
		buf[0] = ADIOL100_SPI_CMD(dev->chip_addr, reg, ADIOL100_WRITE);
		no_os_put_unaligned_be16(value, &buf[1]);
		len = ADIOL100_SPI_DIRECT_LEN;
	}
...
int adiol100_load_msg(struct adiol100_dev *dev, enum adiol100_channel ch,
		      const uint8_t *data, uint8_t txbytes, uint8_t rxbytes,
		      enum adiol100_keep_msg keep)
{
	uint8_t buf[ADIOL100_TX_MAX_LEN + 1]; /* +1 for SPI command byte */
	uint8_t write_len;
	uint8_t read_len;
	uint16_t data_reg;
	uint16_t fc1_reg;
	...
	if (txbytes > ADIOL100_TX_MSG_MAX_LEN)
		return -EINVAL;

	if (rxbytes > ADIOL100_RX_MSG_MAX_LEN)
		return -EINVAL;

	write_len = ADIOL100_TX_HDR_LEN + txbytes;
	read_len = ADIOL100_RX_HDR_LEN + rxbytes;
	...
	buf[0] = ADIOL100_SPI_CMD(dev->chip_addr, data_reg, ADIOL100_WRITE);
	buf[1] = dev->msg_id[ch]++;
	buf[2] = rxbytes;
	buf[3] = txbytes;
	memcpy(&buf[4], data, txbytes);

	/* +1 for SPI command byte */
 ...
 * @param dev      - The device structure.
 * @param ch       - Channel selection (A or B).
 * @param data     - Buffer to store response bytes.
 * @param data_len - Size of @p data buffer. If larger than the FIFO level,
 *                   only the available bytes are read. If smaller than FIFO
 *                   level, this will produce an error.
 *
 * @return 0 in case of success, negative error code otherwise.
 */
		      ...
		      uint8_t *data, uint8_t data_len)
{
	uint16_t rxstat;
	uint8_t fifo_lvl;
	uint8_t buf[ADIOL100_RX_MAX_LEN + 1] = {0}; /* +1 for SPI command byte */
	uint16_t stat_reg;
	uint16_t data_reg;
	int ret;
	...
	if (data_len < fifo_lvl) {
		return -EINVAL;
	}

	buf[0] = ADIOL100_SPI_CMD(dev->chip_addr, data_reg, ADIOL100_READ);

	/* +1 for SPI command byte; RX header not counted in fifo_lvl */
	ret = no_os_spi_write_and_read(dev->spi_desc, buf,
				       fifo_lvl + ADIOL100_RX_HDR_LEN + 1);
	if (ret)
		return ret;
	...
	/* buf[0]=SPI cmd, buf[1]=MsgID, buf[2]=RxBytesAct, buf[3..]=data */
				  ...
				  uint8_t ar_time, uint8_t ar_count)
{
	...
	/* LPFetPwrCfg */
	ret = adiol100_update(dev, pwrcfg_reg, ADIOL100_LPFETPWRMAX_MSK,
			      no_os_field_prep(ADIOL100_LPFETPWRMAX_MSK, pwr_max));
	if (ret)
		return ret;
	...
	ret = adiol100_update(dev, currcfg_reg,
			      ADIOL100_LPCURRMAX_MSK | ADIOL100_LPCURRMIN_MSK, val);
	if (ret)
		return ret;
	...
	ret = adiol100_update(dev, protect_reg,
			      ADIOL100_LPOLTIMOUT_MSK | ADIOL100_LPCLTIMOUT_MSK, val);
	if (ret)
		return ret;
	...
	ret = adiol100_update(dev, slope_reg,
			      ADIOL100_LPSLOPE_MSK | ADIOL100_LPSLOPEBL_MSK, val);
	if (ret)
		return ret;
	...
	return adiol100_update(dev, retry_reg,
			       ADIOL100_LPARTIM_MSK | ADIOL100_LPAR_MSK, val);
}
				      ...
				      uint8_t ar_time, uint8_t ar_count)
{
	...
	/* CQFETPwrCfg */
	ret = adiol100_update(dev, pwrcfg_reg, ADIOL100_CQFETPWRMAX_MSK,
			      no_os_field_prep(ADIOL100_CQFETPWRMAX_MSK, pwr_max));
	if (ret)
		return ret;
	...
	ret = adiol100_update(dev, currcfg_reg,
			      ADIOL100_CQCURRMAX_MSK | ADIOL100_CQCURRMIN_MSK, val);
	if (ret)
		return ret;
	...
	ret = adiol100_update(dev, protect_reg,
			      ADIOL100_CQOLTIMOUT_MSK | ADIOL100_CQCLTIMOUT_MSK, val);
	if (ret)
		return ret;
	...
	ret = adiol100_update(dev, slope_reg,
			      ADIOL100_CQFETSLOPE_MSK | ADIOL100_CQFETSLOPEBL_MSK,
			      val);
	if (ret)
		return ret;
	...
	return adiol100_update(dev, retry_reg,
			       ADIOL100_CQARTIM_MSK | ADIOL100_CQAR_MSK, val);
}

/**
 * @brief Configure the IO-Link framer (FramCtrl1 + FramCtrl2).
 * @param dev           - The device structure.
 * @param ch            - Channel selection (A or B).
 * @param ins_chks      - Enable or disable checksum insertion.
 * @param framer_en     - Enable or disable the framer.
 * @param ckt_err_inh   - Suppress or send message on CKT error.
 * @param msg_repeat    - Auto-repeat last message if FIFO not reloaded.
 * @param bdelay        - Additional byte delay (0–3 TBIT).
 * @param ddelay        - Device answer delay. Wait time = 4 + ddelay * 2
 *                        TBIT. Set 3 for IO-Link standard (10 TBIT).
 *                        Range 0–255.
 * @param tsize_err_inh - Inhibit TX size error checking.
 * @return 0 in case of success, negative error code otherwise.
 */
int adiol100_config_framer(struct adiol100_dev *dev, enum adiol100_channel ch,
			   ...
			   enum adiol100_ckt_err_inh ckt_err_inh,
			   enum adiol100_msg_repeat msg_repeat,
			   enum adiol100_bdelay bdelay,
			   uint8_t ddelay,
			   enum adiol100_tsize_err_inh tsize_err_inh)
{
	uint16_t fc1_reg, fc2_reg;
	uint16_t mask;
	uint16_t val;
	int ret;
	...
	if (ch == ADIOL100_CH_A) {
		fc1_reg = ADIOL100_REG_FRAMCTRL1_A;
		fc2_reg = ADIOL100_REG_FRAMCTRL2_A;
	} else {
		fc1_reg = ADIOL100_REG_FRAMCTRL1_B;
		fc2_reg = ADIOL100_REG_FRAMCTRL2_B;
	}
	...
	ret = adiol100_update(dev, fc1_reg, mask, val);
	if (ret)
		return ret;

	mask = ADIOL100_TSIZEERRINH | ADIOL100_CKTERRINH |
	       ADIOL100_MSGREPEAT | ADIOL100_BDELAY_MSK |
	       ADIOL100_DDELAY_MSK;
	val = no_os_field_prep(ADIOL100_TSIZEERRINH, tsize_err_inh) |
	      no_os_field_prep(ADIOL100_CKTERRINH, ckt_err_inh) |
	      no_os_field_prep(ADIOL100_MSGREPEAT, msg_repeat) |
	      no_os_field_prep(ADIOL100_BDELAY_MSK, bdelay) |
	      no_os_field_prep(ADIOL100_DDELAY_MSK, ddelay);

	return adiol100_update(dev, fc2_reg, mask, val);
}
...
int adiol100_set_burst_len(struct adiol100_dev *dev, enum adiol100_channel ch,
			   uint8_t write_len, uint8_t read_len)
{
	uint16_t reg;
	uint16_t val;
	...
	val = no_os_field_prep(ADIOL100_BURST_RDLEN_MSK, read_len) |
	      no_os_field_prep(ADIOL100_BURST_WRLEN_MSK, write_len);
			   ...
			   uint8_t value)
{
	...
	return adiol100_update(dev, reg, ADIOL100_CYCLTMR_MSK,
			       no_os_field_prep(ADIOL100_CYCLTMR_MSK, value));
}
			     ...
			     enum adiol100_wdg_lock lock)
{
	...
	mask = ADIOL100_WDGTIMEBASE_MSK | ADIOL100_WDGTIME_MSK |
	       ADIOL100_WDGLOCK | ADIOL100_WDGMODE |
	       ADIOL100_WDGENABLE;
	...
	return adiol100_update(dev, ADIOL100_REG_WATCHDOG, mask, val);
}
...
int adiol100_reset_adc_stats(struct adiol100_dev *dev)
{
	...
	uint16_t mask = ADIOL100_TEMPRESSTAT | ADIOL100_RESV24STAT |
			ADIOL100_RESLPSTATB | ADIOL100_RESLPSTATA |
			ADIOL100_RESCQSTATB | ADIOL100_RESCQSTATA;

	return adiol100_update(dev, ADIOL100_REG_CQMEASCONF, mask, mask);
}
				...
				enum adiol100_led_brightness green_bgt)
{
	...
	case ADIOL100_LED1:
		...
		mask = ADIOL100_LED_LO_GREEN_MSK | ADIOL100_LED_LO_RED_MSK;
		val = no_os_field_prep(ADIOL100_LED_LO_GREEN_MSK, green_bgt) |
		      no_os_field_prep(ADIOL100_LED_LO_RED_MSK, red_bgt);
		break;
	case ADIOL100_LED2:
		reg = ADIOL100_REG_LED12BGT;
		mask = ADIOL100_LED_HI_GREEN_MSK | ADIOL100_LED_HI_RED_MSK;
		val = no_os_field_prep(ADIOL100_LED_HI_GREEN_MSK, green_bgt) |
		      no_os_field_prep(ADIOL100_LED_HI_RED_MSK, red_bgt);
		break;
	case ADIOL100_LED3:
			      ...
			      enum adiol100_led_seq green_seq)
{
	...
	gseq_reg = rseq_reg + ADIOL100_LED_GSEQ_REG_OFFSET;
...
int adiol100_estcom(struct adiol100_dev *dev, enum adiol100_channel ch)
{
	...
	uint16_t irq;
	for (int i = 0; i < ADIOL100_ESTCOM_TIMEOUT_MS; i++) {
		no_os_mdelay(1);
