/***************************************************************************//**
 *   @file   adiol100.h
 *   @brief  ADIOL100 IO-Link transceiver driver definitions.
...
#define ADIOL100_BURST_RDLEN_MSK        NO_OS_GENMASK(15, 8)
#define ADIOL100_BURST_WRLEN_MSK        NO_OS_GENMASK(7, 0)
...
/* EstCom timeout (ms) — 3 attempts × 50ms sequence = 150ms worst case */
#define ADIOL100_ESTCOM_TIMEOUT_MS      160
...
#define ADIOL100_TSIZEERRINH            NO_OS_BIT(14)
#define ADIOL100_TXFIFORSTN             NO_OS_BIT(13)
#define ADIOL100_RXFIFORSTN             NO_OS_BIT(12)
#define ADIOL100_CKTERRINH              NO_OS_BIT(11)
#define ADIOL100_MSGREPEAT              NO_OS_BIT(10)
#define ADIOL100_BDELAY_MSK             NO_OS_GENMASK(9, 8)
#define ADIOL100_DDELAY_MSK             NO_OS_GENMASK(7, 0)
...
/* Channel B ADC registers are offset by 0x10 from channel A */
#define ADIOL100_ADC_CHB_OFFSET         0x10
...
/* LED brightness bit masks (LED12BGT / LED34BGT) */
#define ADIOL100_LED_HI_GREEN_MSK       NO_OS_GENMASK(15, 12)
#define ADIOL100_LED_HI_RED_MSK         NO_OS_GENMASK(11, 8)
#define ADIOL100_LED_LO_GREEN_MSK       NO_OS_GENMASK(7, 4)
#define ADIOL100_LED_LO_RED_MSK         NO_OS_GENMASK(3, 0)
...
/* TX FIFO: MsgID + RxBytes + TxBytes header, max payload MC+CKT+32OD+32PD */
#define ADIOL100_TX_HDR_LEN             3
#define ADIOL100_TX_MSG_MAX_LEN         66
#define ADIOL100_TX_MAX_LEN             (ADIOL100_TX_HDR_LEN + ADIOL100_TX_MSG_MAX_LEN)

/* RX FIFO: MsgID + RxBytesAct header, max payload 32OD+32PD+CKS */
#define ADIOL100_RX_HDR_LEN             2
#define ADIOL100_RX_MSG_MAX_LEN         65
#define ADIOL100_RX_MAX_LEN             (ADIOL100_RX_HDR_LEN + ADIOL100_RX_MSG_MAX_LEN)

/* Chip address range: 2-bit field, 0–3 */
#define ADIOL100_CHIP_ADDR_MAX          3

/* LED green sequence register is always red + 1 */
#define ADIOL100_LED_GSEQ_REG_OFFSET   1

/* SPI command byte: [chip_addr:2][reg:5][rw:1] */
#define ADIOL100_READ                   1
#define ADIOL100_WRITE                  0
#define ADIOL100_ADDR_SHIFT             6
#define ADIOL100_REG_SHIFT              1
#define ADIOL100_SPI_CMD(addr, reg, rw) \
	(((addr) << ADIOL100_ADDR_SHIFT) | ((reg) << ADIOL100_REG_SHIFT) | (rw))

/* SPI frame lengths */
#define ADIOL100_SPI_DIRECT_LEN         3
#define ADIOL100_SPI_PAGED_LEN          4

/* Paged register detection */
#define ADIOL100_REG_PAGE_MSK           0xFF00
#define ADIOL100_REG_PAGE_OFFSET(reg)   ((reg) & 0xFF)
...
/* CKT error inhibit: 0 = send message despite CKT error, 1 = discard. */
enum adiol100_ckt_err_inh {
	ADIOL100_CKTERR_SEND    = 0,
	ADIOL100_CKTERR_DISCARD = 1,
};

/* Message repeat: re-send last message on invalid device response. */
enum adiol100_msg_repeat {
	ADIOL100_MSGREP_DIS = 0,
	ADIOL100_MSGREP_EN  = 1,
};

/* Additional byte delay: extra TBIT intervals between device UART bytes. */
enum adiol100_bdelay {
	ADIOL100_BDELAY_0TBIT = 0,
	ADIOL100_BDELAY_1TBIT = 1,
	ADIOL100_BDELAY_2TBIT = 2,
	ADIOL100_BDELAY_3TBIT = 3,
};

/*
 * TX size error inhibit:
 * 0 = send message despite TSizeErr, 1 = discard.
 */
enum adiol100_tsize_err_inh {
	ADIOL100_TSIZEERR_SEND    = 0,
	ADIOL100_TSIZEERR_DISCARD = 1,
};
...
struct adiol100_init_param {
	...
	/* Device address (0-3) set by ADRSEL pin - J10 jumper on ADIOL100EVKIT. */
	uint8_t chip_addr;
	/* Clock source (see enum adiol100_clock_src). */
	uint8_t clock_src;
	/* Clock divider (ClkDiv[1:0]). */
	uint8_t clk_div;
	/* Optional IRQ GPIO for global interrupts (NULL = not used). */
	struct no_os_gpio_init_param *gpio_irqg;
	/* Optional IRQ GPIO for channel A (NULL = not used). */
	struct no_os_gpio_init_param *gpio_irqa;
	/* Optional IRQ GPIO for channel B (NULL = not used). */
	struct no_os_gpio_init_param *gpio_irqb;
};
...
struct adiol100_dev {
	...
	uint8_t chip_addr;
	/* Auto-incrementing FIFO message ID, one per channel. */
	uint8_t msg_id[ADIOL100_NUM_CHANNELS];
	/* IRQ GPIO descriptors (NULL if not configured). */
	struct no_os_gpio_desc *gpio_irqg;
	struct no_os_gpio_desc *gpio_irqa;
	struct no_os_gpio_desc *gpio_irqb;
};
...
/** Configure the IO-Link framer (FramCtrl1 + FramCtrl2). */
