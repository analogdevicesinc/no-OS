...
static int maxq10xx_can_do( mbedtls_pk_type_t type )
{
    return( type == MBEDTLS_PK_MAXQ10XX ) || (type==MBEDTLS_PK_ECDSA) ;
}
...
static int maxq10xx_verify_wrap( void *ctx, mbedtls_md_type_t md_alg,
                       const unsigned char *hash, size_t hash_len,
                       const unsigned char *sig, size_t sig_len )
{
    mxq_err_t err;
    size_t rs_len;
    unsigned char rs[66*2];

    //printf("mxq_verify_wrap\n");
    ...
    // Extract coordinates from the signature
    err = extract_ecdsa_sig((unsigned char**)(&sig),sig+sig_len,rs,&rs_len);
    if(err !=0)
        return err;

    err = MXQ_Verify(ALGO_ECDSA_PLAIN, ((mbedtls_mxq_context*) ctx)->key_id, hash, hash_len, rs, (mxq_length) rs_len*2);
    if(err != MXQ_OK){
        return -1;
    }
...
static int maxq10xx_sign_wrap( void *ctx, mbedtls_md_type_t md_alg,
                   const unsigned char *hash, size_t hash_len,
                   unsigned char *sig, size_t sig_size, size_t *sig_len,
                   int (*f_rng)(void *, unsigned char *, size_t),
                   void *p_rng )
{
    ...
    err = MXQ_Sign(ALGO_ECDSA_PLAIN, ((mbedtls_mxq_context*) ctx)->key_id, hash, hash_len, insig,(mxq_length *) sig_len);
    if(err != MXQ_OK){
        return -1;
    }
...
static void *maxq10xx_alloc_wrap( void )
{
    void *ctx = mbedtls_calloc( 1, sizeof( mbedtls_mxq_context ) );


    return( ctx );
}

static void maxq10xx_free_wrap( void *ctx )
{
    mbedtls_free( (mbedtls_mxq_context *) ctx );
}

const mbedtls_pk_info_t mbedtls_maxq10xx_info = {
    MBEDTLS_PK_MAXQ10XX,
    "ECDSA",
    eckey_get_bitlen,     /* Compatible key structures */
    maxq10xx_can_do,
    maxq10xx_verify_wrap,
    maxq10xx_sign_wrap,
    NULL,
    NULL,
