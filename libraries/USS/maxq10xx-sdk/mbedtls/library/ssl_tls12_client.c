...
#if defined(__MAXQ10XX_CLI__) && defined(MAXQ108x)
    peer_pk = NULL;
    if( ( ret = mxq_rsaes_pkcs1_v15_encrypt(p, olen, ssl->out_msg + offset + len_bytes,
                                                ssl->session_negotiate->ciphersuite) ) != 0 )
    {
        MBEDTLS_SSL_DEBUG_RET( 1, "mxq_rsaes_pkcs1_v15_encrypt", ret );
        return( ret );
    }
...
#endif /* __MAXQ10XX_CLI__ && MAXQ108x */
...
start_processing:
    ...
    if (mbedtls_ssl_ciphersuite_uses_server_signature(ciphersuite_info)) {
        ...
        {
            switch( md_alg ) {
            case MBEDTLS_MD_SHA256: algoid = ALGO_RSASSAPSSPKCS1_V1_5_SHA256; break;
            case MBEDTLS_MD_SHA384: algoid = ALGO_RSASSAPSSPKCS1_V1_5_SHA384; break;
            case MBEDTLS_MD_SHA512: algoid = ALGO_RSASSAPSSPKCS1_V1_5_SHA512; break;
            default:
                MBEDTLS_SSL_DEBUG_RET( 1, "mxq_verifyECDHEparamsSignature RSA unsupported hash", -1 );
                return( -1 );
            }
        }
        else
        ...
        {
            switch( md_alg ) {
            case MBEDTLS_MD_SHA512: algoid = ALGO_ECDSA_SHA_512; break;
            case MBEDTLS_MD_SHA384: algoid = ALGO_ECDSA_SHA_384; break;
            case MBEDTLS_MD_SHA256: algoid = ALGO_ECDSA_SHA_256; break;
            default:
                MBEDTLS_SSL_DEBUG_RET( 1, "mxq_verifyECDHEparamsSignature ECDSA unsupported hash", -1 );
                return( -1 );
            }
        }
#if defined(__MAXQ10XX_CLI__) && defined(MAXQ108x)
        if( ciphersuite_info->key_exchange == MBEDTLS_KEY_EXCHANGE_DHE_RSA )
        {
            MBEDTLS_SSL_DEBUG_MSG( 3, ( "mxq_verifyDHEparamsSignature" ) );
            if( (ret = mxq_verifyDHEparamsSignature(
                    algoid,
                    ciphersuite_info->id,
                    ssl->handshake->randbytes, 64,
                    params, params_len,
                    p, sig_len)) )
            {
                MBEDTLS_SSL_DEBUG_RET( 1, "mxq_verifyECDHEparamsSignature", ret );
                return( ret );
            }
        }
        else
#endif /* __MAXQ10XX_CLI__ && MAXQ108x */
        {
            MBEDTLS_SSL_DEBUG_MSG( 3, ( "mxq_verifyECDHEparamsSignature" ) );
            if( (ret = mxq_verifyECDHEparamsSignature(
...
#if defined(__MAXQ10XX_CLI__) && defined(MAXQ108x)
        MBEDTLS_SSL_DEBUG_MSG( 3, ( "mxq_dh_calc" ) );
        if( (ret = mxq_dh_calc(NULL, &content_len, &ssl->out_msg[header_len], ciphersuite_info->id)) != 0 )
        {
            MBEDTLS_SSL_DEBUG_RET( 1, "mxq_dh_calc", ret );
            return( ret );
        }
#else
...
ecdh_calc_secret:
...
#else
        //TODO: mxq_ecdh_make_public?
        MBEDTLS_SSL_DEBUG_MSG( 3,( "mxq_ecdh_calc" ) );
        if( (ret = mxq_ecdh_calc(&ssl->handshake->ecdh_ctx, &content_len, &ssl->out_msg[header_len], ciphersuite_info->id)) != 0 )
        {
        	MBEDTLS_SSL_DEBUG_RET( 1, "mxq_ecdh_calc", ret );
            return( ret );
        }
#endif /* __MAXQ10XX_CLI__ */
#endif /* MBEDTLS_USE_PSA_CRYPTO */
    ...
    if (mbedtls_ssl_ciphersuite_uses_psk(ciphersuite_info)) {
...
#if defined(__MAXQ10XX_CLI__) && defined(MAXQ108x)
            MBEDTLS_SSL_DEBUG_MSG( 3, ( "mxq_dh_calc" ) );
            ret = mxq_dh_calc( &ssl->handshake->dhm_ctx, &content_len, &ssl->out_msg[header_len], ciphersuite_info->id );
#else
            ret = mbedtls_dhm_make_public(&ssl->handshake->dhm_ctx,
                                          (int) mbedtls_dhm_get_len(&ssl->handshake->dhm_ctx),
                                          &ssl->out_msg[header_len], content_len,
        ...
        if (ciphersuite_info->key_exchange == MBEDTLS_KEY_EXCHANGE_ECDHE_PSK) {
            ...
            MBEDTLS_SSL_DEBUG_MSG( 3, ( "mxq_ecdh_calc" ) );
            ret = mxq_ecdh_calc( &ssl->handshake->ecdh_ctx, &content_len, &ssl->out_msg[header_len], ciphersuite_info->id );
#else
            ret = mbedtls_ecdh_make_public(&ssl->handshake->ecdh_ctx,
                                           &content_len,
...
#ifdef __MAXQ10XX_CLI__
    ssl->out_msg[5] = MBEDTLS_SSL_SIG_ECDSA;
    MBEDTLS_SSL_DEBUG_MSG( 2, ( "signing with ECDSA, hash=%d, sig=%d", ssl->out_msg[4], ssl->out_msg[5] ) );
#else
