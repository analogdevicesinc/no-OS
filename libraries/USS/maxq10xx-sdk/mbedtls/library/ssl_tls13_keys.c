    ...
    unsigned char *buf, size_t buf_len)
{
    ...
    MXQ_PRINTF("maxq_tls13_hkdf_expand() -> %.*s\n", (int)label_len, label);
    status = (psa_status_t)maxq_tls13_hkdf_expand(hash_alg,
                                    secret, secret_len,
                                    hkdf_label, hkdf_label_len,
                                    buf, buf_len,
                                    maxq_tls13_hkdf_get_side());
#else
...
#if defined(__MAXQ10XX_CLI__) && defined(MAXQ108x)
    maxq_tls13_hkdf_set_side(MBEDTLS_SSL_IS_CLIENT);
#endif /* __MAXQ10XX_CLI__ && MAXQ108x */
...
#if defined(__MAXQ10XX_CLI__) && defined(MAXQ108x)
    maxq_tls13_hkdf_set_side(MBEDTLS_SSL_IS_SERVER);
#endif /* __MAXQ10XX_CLI__ && MAXQ108x */
...
#if defined(__MAXQ10XX_CLI__) && defined(MAXQ108x)
    size_t ikmLen  = (memcmp(l_input, all_zeroes_input, l_input_len) == 0) ? 0 : l_input_len;
    size_t saltLen = (memcmp(tmp_secret, all_zeroes_input, hlen) == 0) ? 0 : hlen;

    ret = maxq_tls13_hkdf_extract(secret_new,
                    tmp_secret, saltLen,
                    (unsigned char *)l_input, ikmLen,
                    hash_alg);
#else
    status = psa_key_derivation_input_bytes(&operation,
...
#if defined(__MAXQ10XX_CLI__) && defined(MAXQ108x)
    ret = maxq_tls13_hmac_set_key(hash_alg);
    if (ret) {
        goto exit;
    }

    ret = maxq_tls13_hmac_update(transcript, hash_len);
    if (ret) {
        goto exit;
    }

    ret = maxq_tls13_hmac_final(dst, dst_len);
#else
...
#if defined(__MAXQ10XX_CLI__) && defined(MAXQ108x)
            ret = maxq_tls13_ecc_shared_secret(ssl, shared_secret, &shared_secret_len);
            if (ret) {
                goto cleanup;
            }
#else
