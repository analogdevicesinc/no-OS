...
static inline mbedtls_mxq_context *mbedtls_pk_mxq( const mbedtls_pk_context pk )
{
    return( (mbedtls_mxq_context *) (pk).pk_ctx );
}

int mbedtls_pk_set_maxq10xx_key( mbedtls_pk_context *pkey, int key_object_id, int curveid )
{
    int ret;

    if( ( ret = mbedtls_pk_setup( pkey, mbedtls_pk_info_from_type( MBEDTLS_PK_MAXQ10XX ) ) ) != 0 )
    {
        return( ret );
    }

    mbedtls_pk_mxq(*pkey)->key_id=key_object_id;
    if( ( ret = mbedtls_ecp_group_load( &mbedtls_pk_mxq(*pkey)->grp, curveid ) ) != 0 )
    {
       return( ret );
    }
