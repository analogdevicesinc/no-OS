...
#ifdef __MAXQ10XX__
#ifdef MAXQ108x
    if( mbedtls_pk_get_type( ctx->key ) == MBEDTLS_PK_RSA ) {
        if( ( ret = mxq_pk_sign_rsa( ctx->md_alg, hash, 0, sig, &sig_len, *(mxq_u2 *)param, mbedtls_pk_rsa(*(ctx->key))->padding ) ) != 0 ) {
            return( ret );
        }
    } else
#endif /* MAXQ108x */
    {
        if( ( ret = mxq_pk_sign_ec( ctx->md_alg, hash, 0, sig, &sig_len, *(mxq_u2 *)param ) ) != 0 ) {
            return( ret );
        }
    }
#else
