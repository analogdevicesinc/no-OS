...
hmac_failed_etm_disabled:
    ...
    if (ssl_mode == MBEDTLS_SSL_MODE_AEAD) {
        ...
        #ifdef MAXQ108x
        if( transform->tls_version == MBEDTLS_SSL_VERSION_TLS1_3 ) {
            if( ( ret = maxq_tls13_record_processing( &transform->cipher_ctx_enc,
                    iv, transform->ivlen,
                    add_data, add_data_len,
                    data, rec->data_len,                     /* src */
                    data, rec->buf_len - (data - rec->buf),  /* dst */
                    &rec->data_len,
                    transform->taglen, 1 ) ) != 0 )
            {
                MBEDTLS_SSL_DEBUG_RET( 1, "maxq_tls13_record_processing", ret );
                return( ret );
            }
        }
        else
        #endif /* MAXQ108x */
        if( ( ret = maxq_cipher_aead_encrypt( &transform->cipher_ctx_enc,
                   iv, transform->ivlen,
                   add_data, add_data_len,
                   data, rec->data_len,                     /* src */
                   data, rec->buf_len - (data - rec->buf),  /* dst */
                   ...
                   transform->taglen ) ) != 0 )
        {
            MBEDTLS_SSL_DEBUG_RET( 1, "maxq_cipher_aead_encrypt", ret );
            return( ret );
        }
    #else
...
#else
    #ifdef __MAXQ10XX_CLI__
        if( ( ret = maxq_cipher_cbc_encrypt( &transform->cipher_ctx_enc,
                                   transform->iv_enc,
                                   transform->ivlen,
                                   data, rec->data_len,
                                   add_data, add_data_len,
                                   ...
                                   data, &olen ) ) != 0 )
        {
            MBEDTLS_SSL_DEBUG_RET( 1, "maxq_cipher_cbc_encrypt", ret );
            return( ret );
        }
    #else
                            ...
                            mbedtls_record *rec)
{
    ...
    if (ssl_mode == MBEDTLS_SSL_MODE_AEAD) {
        ...
        #ifdef MAXQ108x
        if( transform->tls_version == MBEDTLS_SSL_VERSION_TLS1_3 ) {
            if( ( ret = maxq_tls13_record_processing( &transform->cipher_ctx_dec,
                    iv, transform->ivlen,
                    add_data, add_data_len,
                    data, rec->data_len + transform->taglen,          /* src */
                    data, rec->buf_len - (data - rec->buf), &olen,    /* dst */
                    transform->taglen, 0 ) ) != 0 )
            {
                MBEDTLS_SSL_DEBUG_RET( 1, "maxq_tls13_record_processing", ret );
        ...
        #endif /* MAXQ108x */
        if( ( ret = maxq_cipher_aead_decrypt( &transform->cipher_ctx_dec,
                  iv, transform->ivlen,
                  add_data, add_data_len,
                  data, rec->data_len + transform->taglen,          /* src */
                  data, rec->buf_len - (data - rec->buf), &olen,    /* dst */
                  transform->taglen ) ) != 0 )
        {
            MBEDTLS_SSL_DEBUG_RET( 1, "maxq_cipher_aead_decrypt", ret );
...
hmac_failed_etm_enabled:
    ...
    #ifdef __MAXQ10XX_CLI__
        ssl_extract_add_data_from_record( add_data, &add_data_len, rec,
                                          transform->tls_version, transform->taglen );

        if( ( ret = maxq_cipher_cbc_decrypt( &transform->cipher_ctx_dec,
                                   transform->iv_dec, transform->ivlen,
                                   data, rec->data_len,
                                   add_data, add_data_len, transform->maclen,
                                   data, &olen ) ) != 0 )
        {
            MBEDTLS_SSL_DEBUG_RET( 1, "maxq_cipher_cbc_decrypt", ret );
            return( ret );
        }
    #else
