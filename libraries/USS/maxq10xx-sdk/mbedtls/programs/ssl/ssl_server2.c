...
#ifdef __MAXQ10XX__
    if((ret = MXQ_Module_Init()) != 0)
    {
        mbedtls_printf( " failed\n  ! MXQ_Module_Init returned -0x%x\n", -ret );
        goto exit;
    }
#endif /* __MAXQ10XX__ */
...
#elif defined(__MAXQ10XX__)
        ret = mxq_x509_root_cert_read(&cacert, ROOTCACERTOBJID);
...
#ifdef __MAXQ10XX__
        if( ( ret = mxq_x509_cli_cert_read(&srvcert, CLICERTOBJID) ) != 0 )
        {
                mbedtls_printf( " failed\n  !  mxq_x509_cli_cert_read returned -0x%x\n\n",
                                (unsigned int) -ret );
                goto exit;
        }

        if( ( ret = mbedtls_pk_set_maxq10xx_key(&pkey, CLIKEYOBJID, MBEDTLS_ECP_DP_SECP256R1) ) != 0 ) {
                mbedtls_printf( " failed\n  !  mbedtls_pk_set_maxq10xx_key returned -0x%x\n\n",
                                (unsigned int) -ret );
        }
