...
static void * aes_gcm_init_hash_subkey(const u8 *key, size_t key_len, u8 *H, u32 *aes_buff)
#else
static void * aes_gcm_init_hash_subkey(const u8 *key, size_t key_len, u8 *H)
#endif
{
	#ifdef _DISABLE_DYNAMIC_MEM_USE_
	void *aes = aes_encrypt_init(key, key_len, aes_buff);
	#else
	void *aes = aes_encrypt_init(key, key_len);
	#endif
	if (aes == NULL)
		return NULL;
	       ...
	       const u8 *aad, size_t aad_len, u8 *crypt, u8 *tag)
{
	...
	#ifdef _DISABLE_DYNAMIC_MEM_USE_
	u8 aes_buff[AES_PRIV_SIZE];
	void *aes = aes_gcm_init_hash_subkey(key, key_len, H, (u32 *)aes_buff);
	#else
	void *aes = aes_gcm_init_hash_subkey(key, key_len, H);
	#endif
	if (aes == NULL)
		return -1;
