 ...
 *   @brief  OSAL implementation — maps rt-labs OSAL functions to FreeRTOS
 *           API calls.
...
{
	osal_timer_ctx_t * ctx;

	if (!xTimer)
		return;

	ctx = (osal_timer_ctx_t *)pvTimerGetTimerID(xTimer);
	if (!ctx || !ctx->callback)
		return;
	...
	bool oneshot)
{
	...
	ctx = pvPortMalloc(sizeof(osal_timer_ctx_t));
