...
#define APP_EVT_PORT_MASK (BIT(IOLINK_NUM_PORTS) - 1)
#define APP_EVT_ALL       ((APP_EVT_PD_0      * APP_EVT_PORT_MASK) | \
			   (APP_EVT_PORTE_0   * APP_EVT_PORT_MASK) | \
			   (APP_EVT_COMLOST_0 * APP_EVT_PORT_MASK))

_Static_assert(IOLINK_NUM_PORTS <= 8,
	       "app_event bit layout supports at most 8 ports");
		   ...
		   uint16_t arg_block_len, arg_block_t *arg_block)
{
	...
	struct iolink_app_port *port;

	if (!m || !arg_block)
		return;

	port = &m->port[portnumber - 1];
...
/* Default PD handler — prints hex bytes to the console. */
...
int iolink_app_init(struct iolink_app_master *ctx, iolink_m_cfg_t *cfg)
{
	...
	if (ctx->handle == NULL) {
		...
		os_event_destroy(ctx->app_event);
		return -EIO;
	}
	...
	for (i = 0; i < cfg->port_cnt; i++) {
		...
		if (!port->event) {
			LOG_ERROR(LOG_STATE_ON,
				  "iolink_app: event create failed for port %u\n",
				  i + 1);
			return -ENOMEM;
		}
		...
		if (!port->mtx) {
			LOG_ERROR(LOG_STATE_ON,
				  "iolink_app: mutex create failed for port %u\n",
				  i + 1);
			os_event_destroy(port->event);
			return -ENOMEM;
		}
...
void iolink_app_run(struct iolink_app_master *ctx)
{
	...
	while (1) {
		...
		if (os_event_wait(ctx->app_event, APP_EVT_ALL,
				  &event_value, 1000))
			continue;
...
int iolink_app_write_param(struct iolink_app_port *port, uint16_t index,
			   uint8_t subindex, const uint8_t *buf, uint8_t len)
{
	if (!port || !buf)
		return -EINVAL;
