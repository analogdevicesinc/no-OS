...
A no-OS application is a stack of four required layers (plus an optional
frameworks layer). Each layer depends only on the one below it through a stable
interface, which is what lets the same
...
.. code-block::
   ...
   ├── linux-userspace/  win/  mac/  ftd2xx/  ...
...
implementation (see :ref:`how-dispatch-works` below).
