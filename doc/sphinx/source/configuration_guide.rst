...
.. code-block:: bash
   ...
   python tools/scripts/no_os_build.py build --project iio_demo --variant iio_wifi --board eval-adicup3029 --clean
...
.. code-block:: bash
   ...
   cmake -B iio_demo --preset max32650fthr -DPROJECT_DEFCONFIG=iio_demo/iio.conf
...
Some projects pull in external libraries — lwIP, FreeRTOS, mbedTLS, LVGL, the
MQTT client, the Azure SDK, the Pico SDK, and a few others. These are not stored
in the repository; they are cloned with Git on demand during the CMake configure
step, at the exact version the build asks for. The requested version of each
library is itself a Kconfig symbol (``CONFIG_FREERTOS_VERSION``,
``CONFIG_LWIP_VERSION``, ``CONFIG_LVGL_VERSION``, ...), so it layers and can be
overridden through the same defconfig/menuconfig/command-line mechanisms as any
other option.

For each library the source is resolved in this order:

1. **Explicit override.** If ``NO_OS_DEP_<LIB>_PATH`` is set (as a CMake variable
   or an environment variable), it is used verbatim — no version check, no
   cloning. Point it at a local working copy when you are developing against a
   library or need a version the defconfig does not offer:

   .. code-block:: bash

      export NO_OS_DEP_LWIP_PATH=~/src/lwip

   The library names follow the version symbols above — ``NO_OS_DEP_LWIP_PATH``,
   ``NO_OS_DEP_FREERTOS_PATH``, ``NO_OS_DEP_MBEDTLS_PATH``,
   ``NO_OS_DEP_LVGL_PATH``, ``NO_OS_DEP_PICO_SDK_PATH``, and so on.

2. **Managed clone.** Otherwise the build clones the requested version once and
   reuses it. By default the clone lands in-tree under ``libraries/`` and is
   reused on every later configure. If ``NO_OS_CACHE_DIR`` is set, the clone goes
   into a shared, version-keyed store under that directory instead — so several
   builds (or CI pipeline runs) reuse the same download rather than re-cloning:

   .. code-block:: bash

      export NO_OS_CACHE_DIR=~/.cache/no-os-deps

An existing clone is reused as-is when it already sits on the requested version,
so switching between builds that ask for different versions re-fetches only what
changed. The in-tree ``libraries/`` clones are git-ignored.
