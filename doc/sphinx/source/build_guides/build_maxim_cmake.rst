...
- **Python 3**. The build creates a private virtual environment under
  ...
  ``tools/scripts/requirements.txt`` (``kconfiglib``, ``pylink-square``).
...
The build system locates the SDK in one of three ways (checked in this order):
...
with ``MAXIM_LIBRARIES environment variable is not defined``.
...
- (Optional) For visual debugging, install Visual Studio Code and the
  ...
  VS Code does not always prompt to open the generated workspace (the
  notification is shown at most once per folder and is suppressed once
  dismissed). Open it explicitly with:
...
``<project>-<variant>-<board>`` at the repo root (override the location with
``--build-dir``). The build artifacts (``.elf``, ``.hex``, ``.bin``) are placed
in ``<build-dir>/build``.
