...
Install Maxim SDK
-----------------
The Maxim Microcontrollers SDK (MSDK) contains the necessary software and tools
to develop firmware for the MAX32xxx and MAX78xxx Microcontrollers. That includes
register and system startup files to enable low-level development for its
supported parts.

- Install the `Maxim Micros SDK <https://www.maximintegrated.com/en/design/software-description.html/swpart=SFW0018720A>`_.
- Set the MAXIM_LIBRARIES environment variable to the MaximSDK/Libraries path
  (the default should be ~/MaximSDK/Libraries):
...
2) Build the project.
    ...
    - The **TARGET** specifies the chip for which the project is built. If it is
      missing, **max32660** will be used. At the moment, the available targets
      are: *max32650*, *max32655*, *max32660*, *max32665*, *max32670*, *max32690*
      and *max78000*.
...
Running/Debugging
-----------------
Once the **.elf**, **.hex** or **.bin** file has been generated, make sure the
board is powered on, JTAG cable connected and use the following commands to
upload the program to the board or debug.
...
Uploading the binary to target is generically achieved with:
    ...
    - The **TARGET** specifies the chip for which the project is built and run.
      If it is missing, **max32660** will be used.
...
projects:
    ...
    You only need to do this once per Git Bash installation:
        - Open up a Git Bash as Administrator once and run the
          **no-os/tools/scripts/git-bash.sh** script.
        - Close the window.
...
- Install the `Maxim Micros SDK (Windows) <https://www.maximintegrated.com/en/design/software-description.html/swpart=SFW0010820A>`_
  to a path without whitespaces like **C:\\MaximSDK**.
...
2) Set the PLATFORM and TARGET environment variables with the following command:
    ...
    - The **TARGET** specifies the chip for which the project is built. If it is
      ...
      are: *max32650*, *max32655*, *max32660*, *max32665*, *max32670*, *max32690*
      and *max78000*.
    - **IMPORTANT**: Setting the environment variables must be done every time a
      new Bash window is opened.
