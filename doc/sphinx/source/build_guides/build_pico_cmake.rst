...
(RP2040) platform using the **CMake** build system. For the legacy Makefile
flow, see :doc:`build_pico`.
...
external SDK install is required. The build generates the SDK headers and the
second-stage bootloader that the SDK's own CMake would normally produce, and
links them into the no-OS executable automatically.
...
From the no-OS repository root, list the configure presets. The ``rpi-pico`` 
preset selects the Raspberry Pi Pico board:
...
Configure with the board preset and select the project/variant with
``PROJECT_DEFCONFIG`` (a path relative to ``projects/`` of the form
``<project>/<variant>.conf``):
