...
which owns the ``iio`` core; the ``iio`` core in turn owns and drives ``iiod``
(the IIO-protocol engine), which decodes each request and calls back into
``iio`` through the ``iiod_ops`` callbacks. The whole thing talks to a host
...
.. code-block::
   ...
   ┌────────────────────────────────────────────────────────┐
   │ iio_app                                                │
   │   iio_app_init(): init the UART, build the             │
   │   device list, then call iio_init()                    │
   │   iio_app_run(): loop over iio_step(), then            │
   │   the optional post_step_callback (custom app code)    │
   └────────────────────────────┬───────────────────────────┘
                                │ iio_step()
                                ▼
   ┌────────────────────────────────────────────────────────┐
   │ iio  — iio_init() creates iio_desc (devices,           │
   │        XML, triggers), sets up the transport,          │
   │        creates the iiod, and wires the                 │
   │        iiod_ops callbacks                              │
   │                                                        │
   │   iio_step() ──► iiod_conn_step()                      │
   │                        │                               │
   │                        ▼                               │
   │          ┌────────────────────────────────┐            │
   │          │ iiod                           │            │
   │          │   parses the IIO protocol and  │            │
   │          │   dispatches each request via  │            │
   │          │   iiod_ops callbacks           │            │
   │          └────────────────────────────────┘            │
   │                                                        │
   │   iiod invokes the iiod_ops callbacks that iio         │
   │   implements: attribute I/O, buffer ops, and           │
   │   send/recv over the transport                         │
   └────────────────────────────┬───────────────────────────┘
                                │ send / recv over transport
                                ▼
   ┌────────────────────────────────────────────────────────┐
   │ transport   UART · USB CDC-ACM · TCP/IP                │
   └────────────────────────────┬───────────────────────────┘
                                │
                                ▼
   ┌────────────────────────────────────────────────────────┐
   │ Client (host)                                          │
   │   iio_info, iio_readdev, iio_writedev,                 │
   │   IIO Oscilloscope, Scopy, pyadi-iio, …                │
   └────────────────────────────────────────────────────────┘

* **iio_app** — the application interface (``iio/iio_app/iio_app.c``).
  ``iio_app_init()`` initializes the UART, assembles the device list, and calls
  ``iio_init()``; ``iio_app_run()`` then drives the main loop, calling
  ``iio_step()`` repeatedly. Every IIO project uses it. The loop is cooperative,
  not blocking: if the application supplies a ``post_step_callback`` (with an
  ``arg``) in its ``iio_app_init_param``, ``iio_app_run()`` calls it once after
  each ``iio_step()`` — so your own application code runs in parallel with the
  IIOD server, without an RTOS, on the same single thread.
* **iio** — the core (``iio/iio.c``). ``iio_init()`` creates the ``iio_desc``,
  registers the ``iio_device`` descriptors, sets up the transport, creates the
  ``iiod`` instance, and wires the ``iiod_ops`` callbacks to its own handlers.
  At runtime it walks the registered devices to read/write attributes and move
  buffered data, and generates the XML that describes the devices to the client.
  ``iio_step()`` services one connection by calling ``iiod_conn_step()``, and
  the ``iiod_ops`` callbacks (attribute I/O, buffer ops, ``send``/``recv``) are
  its implementation of what ``iiod`` invokes.
* **iiod** — the IIO-protocol engine (``iio/iiod.c``, formerly the external
  ``libtinyiiod``). It parses the wire protocol and dispatches each request back
  into ``iio`` through the matching ``iiod_ops`` callback.
