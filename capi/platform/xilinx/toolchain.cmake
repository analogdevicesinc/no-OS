# Xilinx (Vitis) bare-metal toolchain file -- CAPI-LOCAL COPY.
#
# This is capi's own toolchain, selected ONLY for CAPI projects (the root
# CMakeLists.txt sets NO_OS_TOOLCHAIN_PATH here when the project defconfig
# contains CONFIG_CAPI=y). Every other xilinx
# project continues to use drivers/platform/xilinx/toolchain.cmake, which is left
# untouched so CI is unaffected. The two files may diverge: this copy carries the
# Vitis-2025 SDT flow (vitis -s util.py + -DSDT); the drivers one stays classic.
# When all projects migrate to capi, this can become the single source of truth.
#
...
# the HSI-based get_arch helper (tools/scripts/platform/xilinx/util.{py,tcl}),
# then pick the matching bare-metal compiler shipped inside the Vitis install.
# The BSP + linker script are generated later, at configure time, by
...
# Derive the Vitis release year from the install path (e.g. .../Vitis/2025.1 -> 2025),
# mirroring xilinx.mk (VITIS_VERSION/VITIS_YEAR). Vitis >= 2025 uses the SDT flow
# (vitis -s util.py + -DSDT); older uses the classic xsct util.tcl flow.
set(_vitis_year 0)
...
if(NOT DEFINED XILINX_ARCH)
    ...
    set(_util_dir "${CMAKE_CURRENT_LIST_DIR}/../../../tools/scripts/platform/xilinx")
    if(_vitis_year GREATER_EQUAL 2025)
        # SDT flow: use the Vitis-embedded Python (`vitis -s util.py`), NOT plain
        # python3 -- util.py imports hsi/vitis which only exist in that interpreter.
        # argv order matches Make py_util: <fn> <ws> <hw_path> <hw_file> ...;
        # get_arch writes arch.txt to argv[3]=hw_path, so pass _arch_work for both.
        find_program(VITIS_EXECUTABLE vitis HINTS "${XILINX_VITIS}/bin")
        if(NOT VITIS_EXECUTABLE)
            message(FATAL_ERROR "vitis launcher not found under ${XILINX_VITIS}/bin")
        endif()
        execute_process(
            COMMAND ${VITIS_EXECUTABLE} -s "${_util_dir}/util.py"
    ...
    if(NOT _arch_rc EQUAL 0)
        message(FATAL_ERROR "get_arch failed on ${HARDWARE} (rc=${_arch_rc})")
    endif()
...
# Vitis >= 2025 ships the SDT BSP; the headers gate the classic-vs-SDT macro set
# on -DSDT (matches xilinx.mk: CFLAGS += -DSDT). Carried in *_FLAGS_INIT so it
# also reaches CMake's try_compile ABI probe.
set(_xl_extra_flags "")
if(_vitis_year GREATER_EQUAL 2025)
