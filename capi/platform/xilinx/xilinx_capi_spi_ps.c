...
static int ps_async_cleanup(struct capi_spi_xilinx_handle *xh, bool copy_rx)
{
	 ...
	 * path does at the end of a transfer. GPIO CS is driven logically; native
	 * CS is held by FORCE_SSELECT and must be released explicitly, otherwise
	 * the line stays asserted after the frame and a native-CS device sees the
	 * bus as still selected. gpio_cs_device is non-NULL only in GPIO-CS mode,
	 * so it selects which release path applies.
	 */
				  ...
				  struct capi_spi_transfer *transfer)
{
	 ...
	 * The four capi_spi_mode values are just these bits combined, mapped
	 * one-to-one onto XSpiPs options, so all four CAPI modes are supported:
	 *   MODE_0 (CPOL=0,CPHA=0) -> neither option set
	 *   MODE_1 (CPOL=0,CPHA=1) -> XSPIPS_CLK_PHASE_1_OPTION
	 *   MODE_2 (CPOL=1,CPHA=0) -> XSPIPS_CLK_ACTIVE_LOW_OPTION
	 *   MODE_3 (CPOL=1,CPHA=1) -> both options
	 * ps_validate_transfer() rejects any mode bit outside CPHA|CPOL, so an
	 * unknown bit cannot silently select a wrong mode here.
	 */
	u32 options = XSPIPS_MASTER_OPTION | XSPIPS_FORCE_SSELECT_OPTION;
					...
					struct capi_spi_transfer *transfer, int timeout)
{
	...
	/* Translate CAPI SPI mode into XSpiPs options (see capi_spi_ps_transceive). */
	u32 options = XSPIPS_MASTER_OPTION | XSPIPS_FORCE_SSELECT_OPTION;
	if (device->mode & CAPI_SPI_CPHA)
		options |= XSPIPS_CLK_PHASE_1_OPTION;
	if (device->mode & CAPI_SPI_CPOL)
