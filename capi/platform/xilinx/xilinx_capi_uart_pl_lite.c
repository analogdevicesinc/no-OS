		...
		uint32_t len)
{
	...
	if (inst(xh)->SendBuffer.RemainingBytes == 0) {
		 ...
		 * pending it deliberately leaves it untouched. But we disabled
		 * interrupts above before the send, so on that path nothing has
		 * turned them back on and the pending RX would never fire. Re-arm
		 * the shared enable whenever a receive is still outstanding.
		 */
