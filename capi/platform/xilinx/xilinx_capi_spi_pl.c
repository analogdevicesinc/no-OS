...
static int pl_async_cleanup(struct capi_spi_xilinx_handle *xh, bool copy_rx)
{
	...
	/*
	 * Return the chip select to its idle state, matching what the synchronous
	 * path leaves behind. GPIO CS is driven logically; native CS is held by
	 * MANUAL_SSELECT and must be released explicitly, otherwise the line stays
	 * asserted after the frame and a native-CS device sees the bus as still
	 * selected. Deselecting takes two calls: XSpi_SetSlaveSelect updates the
	 * driver's cached mask, XSpi_SetSlaveSelectReg pushes it to the hardware
	 * SSR. gpio_cs_device is non-NULL only in GPIO-CS mode, so it selects which
	 * release path applies.
	 */
...
/*
 * Translate a CAPI SPI mode into AXI Quad SPI (XSpi) option bits.
 *
 * CAPI encodes clock polarity/phase as two independent bits (capi_spi.h):
 *   CAPI_SPI_CPHA (0x01) -> clock phase
 *   CAPI_SPI_CPOL (0x02) -> clock idle polarity
 * The four capi_spi_mode values are just these bits combined, and this maps
 * each bit one-to-one onto the matching XSpi option, so all four CAPI modes
 * are supported:
 *   MODE_0 (CPOL=0,CPHA=0) -> neither option set
 *   MODE_1 (CPOL=0,CPHA=1) -> XSP_CLK_PHASE_1_OPTION
 *   MODE_2 (CPOL=1,CPHA=0) -> XSP_CLK_ACTIVE_LOW_OPTION
 *   MODE_3 (CPOL=1,CPHA=1) -> both options
 * pl_validate_transfer() rejects any mode bit outside CPHA|CPOL, so unknown
 * bits cannot silently select a wrong mode here.
 */
				...
				bool async, bool read_command)
{
	...
	/*
	 * The AXI Quad SPI core runs at the SCLK ratio fixed at synthesis; unlike
	 * the PS controller it has no runtime clock divider to program from a
	 * per-device max_speed_hz. Rather than silently ignore the request and
	 * risk clocking a device faster than it allows, report that per-device
	 * speed control is unsupported so the caller can decide how to proceed.
	 * A zero max_speed_hz means "no constraint" and is accepted.
	 */
				  ...
				  struct capi_spi_transfer *transfer)
{
	...
	/*
	 * Release native CS at the end of the frame, the same way the async
	 * completion path does. MANUAL_SSELECT holds the line until told
	 * otherwise, so leaving it asserted would keep a native-CS device
	 * selected after the transfer. GPIO CS is already deasserted above.
	 */
	if (!use_gpio_cs) {
		(void)XSpi_SetSlaveSelect(inst(xh), XSPI_CAPI_DESELECT_ALL_NATIVE);
		XSpi_SetSlaveSelectReg(inst(xh), inst(xh)->SlaveSelectReg);
			      ...
			      enum capi_spi_cs_control cs)
{
	...
	/* Automatic CS is managed per transfer; nothing to drive here. */
