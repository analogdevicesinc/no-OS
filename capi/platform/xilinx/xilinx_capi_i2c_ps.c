 ...
 * supported: the read path only emits the sub-address as a repeated-start
 * (HOLD) prefix. Callers that set a sub-address but leave repeated_start false
 * would otherwise have the register pointer silently dropped and read from the
 * wrong location, so reject that combination instead.
 */
				...
				struct capi_i2c_transfer *transfer)
{
	...
	if (xh->is_initiator) {
		/*
		 * If HOLD is not already asserted, this is a fresh master frame and
		 * the blocking API may wait for the bus to become available. When a
		 * prior no_stop transfer left HOLD/REP_START set, the bus is
		 * intentionally still owned by this controller, so skip the wait and
		 * continue the combined frame rather than time out on our own hold.
		 */
		if (!(XIicPs_GetOptions(inst(xh)) & XIICPS_REP_START_OPTION)) {
		...
		/*
		 * Target-mode polled send blocks until an initiator addresses this
		 * controller for a read and clocks the bytes out. On a single core
		 * with the peer on the same CPU this would deadlock; use the async
		 * target ops (transmit_async) for the single-board loopback instead.
		 */
		...
		enum capi_i2c_speed speed, uint8_t duty_cycle)
{
	...
	/*
	 * The XIicPs controller supports standard (100 kHz) and fast (400 kHz)
	 * modes only. Fast-plus and above have no hardware support, so report
	 * -ENOTSUP rather than silently clamping to 400 kHz and lying about the
	 * resulting bus rate.
	 */
	switch (speed) {
 ...
 *
 * Target role: arm XIicPs_SlaveRecv; the ISR completes it when an initiator
 * writes to this controller.
 *
 * @return 0 on success, negative errno on failure.
 */
static int capi_i2c_ps_receive_async(struct capi_i2c_device *device,
...
static void capi_i2c_ps_isr(void *handle)
{
	...
	/*
	 * Route to the master or slave handler based on the controller's current
	 * role. is_initiator is not fixed: register_target()/unregister_target()
	 * flip it at runtime, so a controller that was opened as an initiator and
	 * later turned into a target dispatches through the slave handler here.
	 */
	if (xh->is_initiator)
