			      ...
			      const struct capi_timer_config *config)
{
	...
	/*
	 * XTmrCtr_InitHw does not touch the interrupt-occurred flag, so a latch
	 * left set by a prior run (or bootloader) would fire our ISR the instant
	 * IRQs are enabled. In the AXI Timer TCSR that flag is write-1-to-clear:
	 * reading it and writing the same value back with the bit set clears it.
	 * Do this for both channels before any handler is wired up.
	 */
	for (u8 n = 0; n < 2; n++) {
	...
	xh->instance = xi;
	/*
	 * Reconcile the clock from two sources. SDT BSPs put the fabric clock in
	 * Config.SysClockFreqHz; older/legacy configs leave it 0 and rely on the
	 * caller-supplied config->input_clock_hz. Prefer the BSP value when present
	 * (authoritative), otherwise push the caller's value into the XTmrCtr
	 * instance so XTmrCtr_PwmConfigure's tick math has a clock to work with.
	 */
