 ...
 * @brief Maxim (MSDK) PINT (pin interrupt) backend, common to all parts.
 *
 * One PINT port == one MSDK GPIO instance. The port id indexes the MSDK
 * MXC_GPIO_GET_GPIO()/MXC_GPIO_GET_IRQ() helpers, so this source needs no
 * per-part branch: a part with N GPIO ports gets N usable PINT ports.
 *
 * Per-pin callbacks are registered with MSDK (MXC_GPIO_RegisterCallback), which
 * hands the callback a single void*. We pass a per-pin context so the adapter
 * can recover the pin index and the user's CAPI-shaped callback. The GPIO NVIC
 * line is connected through the CAPI IRQ singleton (capi_irq_connect), which
 * requires capi_irq_init() to have run once in main.c.
 */
