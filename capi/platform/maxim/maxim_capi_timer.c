 ...
 *   @brief  Common Maxim (MSDK) CAPI Timer/PWM backend
********************************************************************************
...
/*
 * One backend for every MSDK part. The MSDK MXC_TMR_* API is uniform across
 * parts, so the only per-part variation at the CAPI boundary is which timer
 * instances expose an output pin -- isolated to _max_capi_timer_configure_gpio()
 * and gated on the MAX_CAPI_TIMER_*_PIN_COUNT capability macros (see the header).
 */
 ...
 *
 * The primary (tmrN) and alternate (tmrNb) pin sets available differ by part;
 * cases beyond the per-part MAX_CAPI_TIMER_*_PIN_COUNT are compiled out and
 * return -ENOTSUP so an unpinnable instance fails cleanly instead of failing to
 * link against a gpio_cfg_tmrN symbol its <part>/mxc_pins.h does not define.
 *
		...
		enum max_capi_gpio_vssel vssel)
{
	...
	if (!use_alternate_pin) {
		...
		case 3:
			...
			break;
#if (MAX_CAPI_TIMER_PIN_COUNT > 4)
		case 4:
			gpio_config = gpio_cfg_tmr4;
			break;
#endif
#if (MAX_CAPI_TIMER_PIN_COUNT > 5)
		case 5:
		...
		case 1:
			...
			break;
		case 2:
			gpio_config = gpio_cfg_tmr2b;
			break;
		case 3:
			gpio_config = gpio_cfg_tmr3b;
			break;
#if (MAX_CAPI_TIMER_ALT_PIN_COUNT > 4)
		case 4:
			gpio_config = gpio_cfg_tmr4b;
			break;
#endif
#if (MAX_CAPI_TIMER_ALT_PIN_COUNT > 5)
		case 5:
				  ...
				  const struct capi_timer_channel_config *ch_config)
{
	...
	/*
	 * These timer modes require a timer input pin; the input-capture pin
	 * routing is not modelled by this backend yet.
	 */
 ...
 * 	  Capture mode requires an input pin that this backend does not model,
 * 	  so this returns -ENOTSUP.
 * @param handle The timer handle
 * @param chan The channel (0 = Timer A, 1 = Timer B)
 ...
 * @param handle The timer handle (as a void * so it can also be registered
 ...
 * @param handle The timer handle
 */
static void max_capi_timer_isr_capi(struct capi_timer_handle *handle)
{
	max_capi_timer_isr(handle);
}
