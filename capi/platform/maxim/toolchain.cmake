# Maxim (MSDK) bare-metal toolchain file -- CAPI-LOCAL COPY.
#
# Selected ONLY for CAPI projects: the root CMakeLists.txt points
# NO_OS_TOOLCHAIN_PATH here when the project defconfig contains CONFIG_CAPI=y.
# Every other maxim project keeps using drivers/platform/maxim/toolchain.cmake,
# left untouched so CI is unaffected. Identical to the drivers copy for now; it
# exists so capi/ does not depend on drivers/, which it replaces. The two may
# diverge later.
...
# Linker script selection: standard vs secure boot (SLA) variant.
# For MAX32690 with CONFIG_MAXIM_SECURE_BOOT=y, use the SLA linker script that
# reserves 0x10000000–0x100001FF for the secure application header and places
# the CM4 code at 0x10000200. Otherwise use the standard linker script.
set(LINKER_SCRIPT "${MAXIM_LIBRARIES}/CMSIS/Device/Maxim/MAX${TARGET_NUM}/Source/GCC/${TARGET}.ld")
if(TARGET_NUM EQUAL 32690 AND CONFIG_MAXIM_SECURE_BOOT)
    # Check if the CAPI local copy exists first; fall back to MSDK if not.
    if(EXISTS "${CMAKE_CURRENT_LIST_DIR}/max${TARGET_NUM}/max${TARGET_NUM}_sla.ld")
        set(LINKER_SCRIPT "${CMAKE_CURRENT_LIST_DIR}/max${TARGET_NUM}/max${TARGET_NUM}_sla.ld")
        message(STATUS "Using SLA linker script (secure boot): ${LINKER_SCRIPT}")
        add_compile_definitions(__SLA_FWK__)
    else()
        message(FATAL_ERROR "SLA linker script not found. Expected: "
            "${CMAKE_CURRENT_LIST_DIR}/max${TARGET_NUM}/max${TARGET_NUM}_sla.ld")
    endif()
endif()
...
set(CMAKE_EXE_LINKER_FLAGS "${COMMON_CPU_FLAGS} -specs=nosys.specs -Wl,--gc-sections,--undefined=_sbrk ${MCU_LINKER_FLAGS} \
    -T${LINKER_SCRIPT} --entry=Reset_Handler" CACHE STRING "Linker flags for MCU" FORCE)
