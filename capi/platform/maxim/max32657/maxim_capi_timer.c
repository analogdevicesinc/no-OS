...
static void max_capi_timer_isr(struct capi_timer_handle *handle);
			       ...
			       const struct capi_timer_config *config)
{
	...
	if (use_irq) {
		...
		ret = capi_irq_connect(timer_irq,
				       (capi_isr_callback_t)max_capi_timer_isr,
				       timer_handle);
		if (ret)
			goto free_handle;
 ...
 * @return 0 on success, negative error code otherwise
 */
static int max_capi_timer_channel_init(struct capi_timer_handle *handle,
				       uint32_t chan)
{
	int ret;
	struct max_capi_timer_priv *timer_priv;
 ...
 * @return 0 on success, negative error code otherwise
 */
static int max_capi_timer_register_event_callback(struct capi_timer_handle
		*handle,
 ...
 * @return 0 on success, negative error code otherwise
 */
static int max_capi_timer_channel_register_callback(struct capi_timer_handle
		*handle,
		uint32_t chan,
...
static void max_capi_timer_isr(struct capi_timer_handle *handle)
{
	struct capi_timer_handle *timer_handle = handle;
	struct max_capi_timer_priv *timer_priv;
	uint32_t flags;
