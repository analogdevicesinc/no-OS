...
enum max_capi_gpio_func {
	...
	/*
	 * ME10 exposes only ALT1/ALT2; the higher pad functions do not exist
	 * in its MSDK enum, so they are omitted rather than aliased onto a
	 * function that would silently mux the wrong peripheral.
	 */
#if (TARGET_NUM != 32650)
 ...
 * @brief Maxim platform-specific GPIO extra configuration
 */
struct max_capi_gpio_extra_config {
	/** Voltage selection */
	...
	/**
	 * Optional sparse port mask. Zero (the default) means "derive the mask
	 * from config->num_pins", i.e. the contiguous window P<n>.0..P<n>.N-1.
	 *
	 * Some parts put unrelated functions inside that window: MAX32660 has a
	 * single 14-pin port carrying SWD (P0.0/P0.1), both UARTs and every
	 * serial block, so a contiguous mask makes a port-wide direction or
	 * config write re-mux the debug port and the console. Set this to an
	 * explicit bitmask to restrict every port-wide operation to the pins the
	 * board actually dedicates to GPIO.
	 */
	uint32_t pin_mask;
};
